...
| `compat_global_config_schema_future` | 1 | Guards global-schema remote/cloud policy, offline/local warnings, env/local-satisfied storage, JSON `LBR-CONFIG-001`, secret-free diagnostics, explicit configuration-role bootstrap/cascade and zero-write reads | `src/cli.rs`, `src/command/config.rs`, `src/internal/config.rs`, `src/internal/db/schema.rs`, `src/utils/client_storage.rs`, `src/utils/error.rs` |
