...
| `db_migration_test` | 1 | SQLite schema bootstrap + migration round-trip, including isolated global/system config fixture restoration, M4 import identity/tombstone, M5 subagent content empty up→down→up, non-empty recovery-state rollback refusal, the §C.8 workspace-lease down guard (non-terminal rows refuse, settled rows roll back, live leases block deeper W3 rollbacks transitively), and uniqueness/state failure matrix | `src/internal/db.rs`, `src/utils/test.rs`, `sql/` |
