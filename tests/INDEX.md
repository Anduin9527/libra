...
| `compat_global_config_schema_future` | 1 | Guards global-schema policy and role routing, secret-free read-only doctor, absent/corrupt/WAL no-create diagnostics, receipt provenance and stable human/JSON/docs contracts | `src/cli.rs`, `src/command/config.rs`, `src/command/config/doctor.rs`, `src/internal/config.rs`, `src/internal/db/schema.rs`, `src/utils/client_storage.rs`, `src/utils/error.rs` |
