...
| `code_ui_remote_state_matrix` | 2 | Cross-surface state replication matrix, including mid-turn detach/cancel settling and an explicit wire-v1 legacy status-event fixture | `src/internal/ai/web/code_ui.rs`, `src/command/code_control.rs`, `src/internal/ai/workflow_baseline.rs` |
| `code_ui_remote_security_matrix` | 2 | Auth/token/origin enforcement matrix | `src/command/code_control*.rs` |
| `code_ui_remote_generation_matrix` | 2 | Generation control across surfaces (no live LLM), retaining one explicit wire-v1 full-snapshot SSE assertion | `src/internal/ai/web/`, `src/command/code.rs` |
...
| `code_provider_resolution_test` | 10 | plan-20260825 PS-06 zero/one/many credential detection for `libra code` without `--provider`: zero candidates exit 128 (LBR-AUTH-001) with the a-mode checked-chain guidance and credential-free block; a single candidate auto-selects with the id/env/layer note on stderr only (stdout clean, key values never printed — GC-PS-01); multiple candidates exit 129 (LBR-CLI-002) listing id-sorted candidates with their hit layers. Isolated HOME + LIBRA_CONFIG_GLOBAL_DB per test; the six auto-selectable keys are env_remove'd so ambient credentials cannot leak. terra R1 additions: the --model pairing guard fires before env-file IO (129 with --env-file pointing at a directory), an explicit --provider structurally skips detection probes (poisoned LIBRA_CONFIG_GLOBAL_DB never reached), and mixed process-env + global-vault candidates carry distinct layer labels. terra R2 additions: `--repo B` detects against B's repo-local vault (caller repo A's key never selected), and `--machine` turns the auto-selection note into a structured JSON event (no prose). terra R4: empty-string keys (process env or --env-file) count as unconfigured and land on the zero-candidate guidance; terra R5: an empty upper layer falls through to the usable layer beneath (empty process env -> repo-local vault; empty env-file value -> process env), and boot agrees with the detection verdict | `tests/code_provider_resolution_test.rs`, `src/command/code.rs`, `src/internal/config.rs` |
...
| `code_mcp_dual_entry_test` | 2 | MCP stdio + http dual entry parity, with explicit v1 full-snapshot SSE compatibility assertions | `src/internal/ai/mcp/`, `src/command/code.rs` |
...
| `ai_code_ui_wire_test` | 2 | Wire-format contract including built-in automation wire-v2 negotiation versus the unchanged server v1 omission default, camelCase `threadGraph`, digest-only/raw-note-free `intent_revision_recovery_is_additive_and_pins_sse_snake_case`, dedicated receipt `sse_wire_v2_intent_revision_consumed_uses_dedicated_payload`, catalogued 409 `plan_execution_not_available_is_a_catalogued_conflict`, and typed drift/revision errors in `phase1_workspace_and_revision_errors_are_catalogued` | `src/internal/ai/web/`, `src/internal/ai/agent/`, `src/command/code_control.rs` |
