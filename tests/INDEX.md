...
| `e2e_mcp_flow` | 2 | End-to-end MCP server flow (TA-04: launches with OS-assigned ports `--port 0 --mcp-port 0`, endpoints read back from the startup banner; shutdown asserted by connect-refused, not rebind). MIG-R03: nextest fixes `test_e2e_mcp_flow` and `test_web_only_sigterm_releases_ports` at 20 minutes per attempt, 120s slow warnings and 10s shutdown grace; owned by `tests/NEXTEST_GROUPS.sh`, regenerated into `.config/nextest.toml` (not a `cargo test` timeout) | `src/internal/ai/mcp/`, `tests/NEXTEST_GROUPS.sh`, `.config/nextest.toml` |
