...
| `compat_global_config_schema_future` | 1 | Guards config role policy, secret-free read-only doctor and confirmed attested Unix repair: canonical/private-path rejection, consistent backup/rowsets, 100 MiB budget, new-reader/idempotence and docs; `test-upgrade` adds lock/attestation/concurrent-copy fault gates | `src/cli.rs`, `src/command/config.rs`, `src/command/config/`, `src/internal/config.rs`, `src/internal/db/schema.rs`, `src/utils/client_storage.rs`, `src/utils/error.rs` |
...
| `compat_declined_interactive_flags` | 1 | Guards remaining D15/D16 interactive flags (`add -i`, `commit -p/--interactive`, `restore/checkout/stash -p`, `rebase -r`, checkout/stash `--[no-]auto-advance`) fail with `LBR-UNSUPPORTED-001`, write nothing, keep table-miss flags as `LBR-CLI-002`, and stay off `--help` | `src/cli.rs` |
...
| `db_migration_test` | 1 | SQLite schema bootstrap + migration round-trip, including role-scoped manifests/ledgers, config-only bootstrap, future/Derived zero-write, atomic configuration upgrade failure, attested repair ledger-only preservation and test-upgrade-gated atomic rollback, 1,000-row no-value-read inspection, isolated global/system fixture restoration, M4 import identity/tombstone, M5 subagent content empty up→down→up, non-empty recovery-state rollback refusal, the §C.8 workspace-lease down guard (non-terminal rows refuse, settled rows roll back, live leases block deeper W3 rollbacks transitively), and uniqueness/state failure matrix | `src/internal/db.rs`, `src/internal/db/schema.rs`, `src/utils/test.rs`, `sql/` |
...
| Resume | `code_resume_test::libra_code_resume_is_unknown` | RC-16: public `libra code --resume` is an unknown command |
...
| Generation / CLI entry | `code_ui_remote_generation_matrix::generation_sse_observes_tool_execution_and_final_completion` | generation settles idle |
...
| `code_provider_resolution_test` | 2 | RC-16: `libra code` provider/boot flags are unknown commands | `src/cli.rs` |
| `code_provider_boot_test` | 2 | RC-16: `libra code` boot is an unknown command | `src/cli.rs` |
...
| `code_mcp_dual_entry_test` | 2 | RC-16: `libra code --stdio` is an unknown command | `src/cli.rs` |
| `code_resume_test` | 2 | RC-16: `libra code --resume` is an unknown command | `src/cli.rs` |
...
| `code_codex_runtime_test` | 2 | RC-16: `libra code --provider codex` is an unknown command | `src/cli.rs` |
...
| `e2e_mcp_flow` | 2 | RC-16: `libra code --stdio` / `--web-only` are unknown commands | `src/cli.rs` |
...
| `old_reader_oracle_test` | 1 | Opt-in hash-pinned Linux v0.22.16 refuses Global/System barriers and the repaired actual v0.22.19 producer fixture before writes; new-reader succeeds (`--features test-upgrade`, ignored; repair requires `LIBRA_TEST_PRODUCER_BINARY`) | `src/internal/db.rs`, `src/internal/db/schema.rs`, `src/command/config.rs`, `src/command/config/repair.rs` |
