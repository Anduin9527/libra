...
参考版本：Lore `0.8.7-nightly`（revision `ace4756`，2026-08-25），见 `/Volumes/Data/competition/epicgames/lore/Cargo.toml:26`

Libra 基线：`main` HEAD `89081277a`，`Cargo.toml:3` version `0.21.27`
...
命令面权威来源：Lore 的 clap CLI 在 `/Volumes/Data/competition/epicgames/lore/lore-client/src/cli/`（`cli.rs` + `commands/*.rs`），而非 `lore/src/interface.rs`（后者是 `lore-capi` 的 `extern "C"` C-ABI 面，印证 Lore「API/C-ABI 第一」的产品形态）；命令文档见 Lore 仓内 `docs/reference/lore-cli-commands.md`（**注意**：该文档滞后于 clap 源码——末次更新为 `4ff461b`，2026-08-17，尚未收录 2026-08-21 由 `d04595e` 新增的 `link info`，故「以 clap 源码为准」的规则继续有效）。本次修订已对照 Lore `lore-client` CLI 与 Libra 源码逐条复核（含 §3、附录 A 的事实更正），更正了若干把 Libra 已实现能力误列为缺口、以及把 Lore 命令层级压平的描述。
...
- Lore 当前 workspace 版本为 `0.8.7-nightly`（`Cargo.toml:26`）。
- Lore 命令面已经包含 `status --scan`、`status --check-dirty`、`dirty`、`stage --scan`、`stage --case`、`service`、`notification`、`completions`、`shared-store set-use-automatically`、`branch diff`、`branch reset`、`branch protect`、`branch archive`、`branch metadata`。**2026-08-27 复核仍全部成立**：`repository.rs:64-91`（status `--scan`/`--check-dirty`）、`file.rs:271-321`（stage `--case`/`--scan`）、`cli.rs:245/267/275/300/303/306/309`（Status/Stage/Dirty/Service/Notification/Completions/SharedStore）、`shared_store.rs:34`（`SetUseAutomatically`）。
- **本轮新增的全局 flag（自 `0.8.4` 起的变化）**：`--gc`（「后台跑自动 GC」）已被**移除**，替换为 `--no-gc`（「阻止本命令的自动增量 GC；GC 平时随写入在后台跑，`lore repository gc` 无论如何都跑全量」，`cli.rs:125-127`——`:125` 是被引用的 doc 注释即 clap help 原文，`:126-127` 是 `#[clap(global = true, long = "no-gc", action)]` 与 `no_gc: bool`）——即 Lore 把 GC 从「显式触发」改成了「写入时自动增量 + 按命令抑制」；另新增 `--identity-token` 与 `--access-token`（`cli.rs:83,88`，`4ff461b`，均 `conflicts_with = "identity"`），允许经 argv 注入令牌——与 Libra 1.6 的「无 `--token` flag」是**有意差异**，见 1.6 与附录 A `auth` 行。
- **命令族集合基本稳定**：`branch`/`repository`/`revision`/`file`/`lock` 五个文件的 `enum *Commands` 在 `d57da2f` 与 `ace4756` 完全一致；唯一新增是 `link info`（`d04595e`，2026-08-21，`link.rs:111`）。即本轮 241 个提交的漂移集中在 flag 与实现层，而非命令族增删。
...
- Lore 的 roadmap 明确把可扩展锁、VFS、links/layers、桌面/Web/Unreal 客户端、edge 拓扑、forks/isolated partitions 放在 2026 以后持续推进。**2026-08-27 复核：`docs/roadmap.md` 末次改动为 `20c2c27`（2026-06-17，早于上次校验），全部条目未变。**
...
|---|---|---|---|
| 稀疏/VFS/惰性水合 | 有 bare/shallow、`.libraignore`、tiered LRU、FUSE worktree 基础；**已实现** 只读 sparse VIEW（2.2，`src/command/sparse_view.rs`）、object alternates（2.3，`src/command/alternates.rs`）、整对象 `hydrate`（3.3，`src/command/hydrate.rs`） | **已实现**：sparse view / alternates / 整对象水合。**仍是缺口**：materializing sparse、partial-clone promisor、透明 FUSE VFS（**口径按 plan-long LR-09**：「sparse-view 只读；hydrate 为 whole-object；无 promisor/VFS」，状态「已验证」，P2） | 剩余三项均在 Phase 3 之后 |
...
| typed metadata | 有 notes、config_kv；**已实现** `libra metadata` repo/branch/revision 三作用域 + `--numeric`/`--binary`（`src/command/metadata.rs:88-103`） | **已实现** repo/branch/revision；**仍缺** file 作用域（1.10 显式延后，待 side-tree vs §3.6 红线裁决） | 已交付；file 作用域待裁决 |
...
| 服务端/复制 | Git client only（本地 `libra service` 环回 SSE 服务 `src/command/service.rs` + 只读 publish Worker 不构成对标） | 无 Lore 式**多租户服务端**与自研传输协议（QUIC/gRPC、replication、partition） | 大多推迟，不建议复制 |
...
借鉴 Lore LEP 工艺（见 `/Volumes/Data/competition/epicgames/lore/docs/proposals/README.md`；自上次校验起 LEP 集新增 4 篇——`2026-07-24-tokio-runtime-split-and-async-io.md`、`2026-08-02-resolved-storage-operations.md`、`2026-08-03-fragment-metadata-on-the-s3-object.md`、`2026-08-04-unify-store-existence-and-metadata.md`，均为内部存储/运行时工程，不产生新的用户可见能力缺口），每个 Phase 1/2/3 编号项动工前必须填写并通过下表，缺任一格只能作为 feature-gate 实验保留。§5.1 的全局门禁视为本模板的默认继承项。
...
> 「Libra 当前类比」列已于 **2026-08-27** 全面回填已交付命令（此前多数行停留在规划期的「无直接等价」措辞）。行本身一律保留、不重编号。
...
| `auth` | ✅ 已交付：`libra auth`（`src/command/auth.rs`）+ keyring 后端 + `auth migrate --to` | 1.6、2.7。**有意差异**：Lore `0.8.7` 起有全局 `--identity-token`/`--access-token`（`4ff461b`），Libra 不提供 argv 令牌入口 |
| `layer` | ✅ 已交付：`libra layer`（`LBR-LAYER-001`，`docs/error-codes.md:101`） | 2.4 |
| `link` | submodule/product boundary | 不实现（link/subtree 组合属产品边界，与 submodule 同拒，见 §3.4）。竞品侧本轮新增 `link info` 与 `branch archive --include-links`——不改判 |
...
| `repository store immutable query` | 无直接等价 | 暂不做（Lore 私有 immutable store 查询）。2026-08-27 复核仍成立 |
