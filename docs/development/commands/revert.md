...
- continue consumes the external conclusion marker（#477 HF-02，ADR-HF-03 第 3 条）：`--continue` 读到 `stop_concluded` 时不再把当前索引定稿为被停 revert，而是直接处理剩余提交。
