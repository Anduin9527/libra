...
| mega2 | intentionally-different | intentionally-different（Libra-only） | `libra mega2 browser` 是唯一的 Mega2 远端浏览入口，Git 无等价契约：只读、匿名的单层 `GET /api/v1/tree`；交互模式要求 TTY 且退出必还原终端，`+` 键经 `POST /api/v1/create-entry` 建目录（token 优先序 `--token-file` → `LIBRA_MEGA2_TOKEN` → `--token`，不持久化、不回显）；`--json`/`--machine` 恰好一次 GET、不 POST，token flag 仅 TUI 可用；不读写本地仓库/配置。命令保持 `CommandScope::ReadOnly` 但 `MutationClass::ExternalOrUnknown` |
