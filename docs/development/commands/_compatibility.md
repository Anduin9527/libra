...
| mega2 | intentionally-different | intentionally-different（Libra-only） | `libra mega2 browser` 是唯一的 Mega2 远端浏览入口，Git 无等价契约：只读、匿名的单层 `GET /api/v1/tree`；交互模式要求 TTY 且退出必还原终端，TUI 的 `+`/`d`/`m`/`R` 键各经一次 `create-entry`/`delete-entry`/`move-entry` POST 加一次重载 GET 建/删/移/改名（`d` 需确认行、文件惰性、敌意目标在 POST 前拒绝；token 优先序 `--token-file` → `LIBRA_MEGA2_TOKEN` → `--token`，不持久化、不回显）；`--json`/`--machine` 恰好一次 GET、不 POST，token flag 仅 TUI 可用；不读写本地仓库/配置。命令保持 `CommandScope::ReadOnly` 但 `MutationClass::ExternalOrUnknown` |
...
| Code Web-only / Agent runtime | W4 bake window closed: default Web Code UI is live (W4-01); thread/version graph is in Web Code UI (W4-04); W5-07 removed the deprecated `--web`/`--web-only` aliases and the hidden `LIBRA_CODE_LEGACY_TUI=1` rollback env (old flags now fail with a usage error plus migration hint). W5-06 removed the Code TUI startup path and the bare `--provider codex --resume` legacy TUI resume driver (now a usage error plus migration hint; every `libra code` launch apart from MCP `--stdio` and the `--control stdio` client shim launches the Web Code UI). W5-08 removed the interactive graph TUI entry. **W5-09 shipped the whole W5-01 family as the single breaking release v0.20.0** (last supporting release: v0.19.158); W5-03 retired the `src/internal/tui` module and W5-10 removed the unused ratatui/crossterm dependency graph. W2-03 IntentSpec revision uses Prepared/Active/Claiming/Consuming authority: exact raw `/intent cancel` commits its fixed no-provider effect, while Modify commits only after exactly one successful draft submission and a durable replacement-review marker; malformed order and zero/multiple submissions fail closed without consuming the revision. Fresh explicit-direct input (including padded cancel) is `SESSION_BUSY` while revision authority is active, except exact terminal or matching live `commandId` retries; the 16 KiB Modify limit is checked before Claiming. One linear `ValidatedIntentRevisionReceiptIndex` supplies source/receipt/consumer/marker/status authority for the whole replay; its 5,000-event/700-receipt regression caps relationship visits at four per event. A committed receipt permanently closes exact retry lineage, while restart heals stale cancel/replacement projection without rerunning the provider. W2-03 review contexts keep an exact Execute content fingerprint plus an additive `metadata-v1` resume/pre-write signal token; new bindings use a metadata-before/exact-before/exact-after/metadata-after stable interval, while contexts without that token remain readable and fall back to exact content comparison. Every scan uses 30-second / 1,000,000-traversed-entry / 128-MiB-encoded-path-name cooperative work budgets; directory entries are capped before deterministic sorting, and blocking filesystem calls plus final EOF fail closed after control returns. Path enumeration remains ignore-aware and path-based but is not read authority. Unix authoritative reads use pinned root/parent fds with `openat`/`fstatat`/`readlinkat`, nofollow/nonblock, and identity/reopen checks; Windows uses pinned handles, `FILE_FLAG_OPEN_REPARSE_POINT`, final path/file identity, and `FSCTL_GET_REPARSE_POINT`; unsupported platforms fail closed. Startup rejects sequence gaps/window cuts before gate authority or context GC; the persistent OS append lock is never age-reclaimed/unlinked and refuses symlink or special entries. A distinct process-lifetime Phase 1 writer lease is acquired before reload/fold, binds the exact store/session and permits one persistence claim; it is separate from append/controller locks, with Unix nofollow/nonblock + dev/inode and Windows reparse + volume/file-identity checks. Checkout/workspace signals are recoverable gate state; metadata-only warnings may pass Execute's exact recheck. W2-04 confirmed-plan execution in the default Web runtime is wired through `submit_confirmed_plan_execution`; `WorkflowPattern` and per-workflow budget remain open. | `CodeArgs` no longer exposes `--web/--web-only` (W5-07); default `execute()` is Web-only (`code_uses_web_launch = !stdio`), and bare `--provider codex --resume` is rejected by `validate_mode_args` (W5-06). `validated_intent_revision_consumption_receipts` returns the shared `ValidatedIntentRevisionReceiptIndex`; `source_terminals`/`exact_source_terminal`, `receipts`/`exact_receipt_for_source`, and `committed_consumer_status` consume it. `Phase1CheckoutBinding::{capture,workspace_matches,workspace_change_matches}` separates content authority from the resume/pre-write signal token; stable capture refuses mixed exact/metadata baselines; `prepare_phase1_recovery_authority` consumes one complete committed replay before authority validation and GC; `HeadlessSessionPersistence::acquire_session_lease` precedes resume load/fold; `phase1_workspace_warning`, `PHASE1_WORKSPACE_CHANGED`, `SESSION_BUSY`, `INVALID_QUERY_PARAM`, and `PLAN_REVISION_NOTE_REQUIRED` pin recoverable drift and unconsumed revision behavior. Network Allow currently returns `PLAN_EXECUTION_NOT_AVAILABLE` without starting mutation because the W2-04 confirmed-plan execution handoff is not implemented. `BudgetScope` is still Session/Agent/Goal; no `WorkflowPattern` layer. | W5-01 family deleted the deprecated public surfaces after the 3-patch bake (W5-01 code-control shim, W5-07 web aliases/rollback env, W5-06 TUI startup path, W5-08 graph entry) and shipped them as v0.20.0 (W5-09). W5-03 retired the TUI module; W5-10 removed its unused rendering dependencies without changing public surface. Keep the legacy Phase 1 content-only reader and do not treat a `PHASE1_WORKSPACE_CHANGED` drift 409 as execution parity. W2-04 confirmed-plan execution is implemented and tested; Network Allow admits the serialized runtime queue instead of returning `PLAN_EXECUTION_NOT_AVAILABLE`; remaining AG work is in `docs/development/internal/code-agent-runtime.md`. |
