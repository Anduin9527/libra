...
- 2026-07-11（plan-20260708 P1-05d，sort 片）：`branch.sort` 配置默认接入严格 local→global→system 级联（`configured_branch_sort`，`--sort` 优先）。刻意与 flag 的副作用区分：配置默认在 `has_commit_filters`/`show_unborn_head` 判定之后解析，因而不隐含 `--list`、不抑制 unborn-HEAD 行（Git 行为）。无效配置值 → `BranchError::InvalidSortConfig`（`LBR-CLI-002`，报错点名 `branch.sort`），读取失败 → `SortConfigRead`（`LBR-IO-001`），均在任何列表输出前。已记录收窄：重复配置值只应用胜出 scope 的最后一个（Git 叠成多键排序）。回归：`compat_config_defaults_semantics` 的 `branch_sort_config_orders_list_and_cli_wins`（含配置下创建分支、多值 last-wins）、`branch_sort_config_keeps_unborn_head_line`（unborn 仓库仍显示当前分支行）、`sort_config_read_failure_is_io_error_before_listing`（不可读 global 库 → LBR-IO-001 点名键）。
...
- Synopsis：`libra branch [-l] [-r] [-a] [--contains [<commit>]] [--no-contains [<commit>]] [--points-at <object>] [--merged [<commit>]] [--no-merged [<commit>]] [--sort <key>] [--ignore-case]` / `libra branch [<new_branch>] [<commit_hash>]` / `libra branch (-d | -D) <branch>` / `libra branch -m [<old_branch>] <new_branch>` / `libra branch (-c | -C) [<old_branch>] <new_branch>` / `libra branch -u <upstream> [<branch>]` / `libra branch --unset-upstream [<branch>]` / `libra branch --show-current`。列表形式额外接受 `[--column[=<MODE>]]`、`[--no-column]` 与 `[-v | --verbose]`（可重复 `-vv`）。
- `#477` HF-07：`-u/--set-upstream-to <upstream> [<branch>]` 按 ADR-HF-08 解析。`<branch>` 缺省当前分支；超过一个额外位置参数为 `too many arguments to set new upstream`（129 / `LBR-CLI-002`）。解析顺序：已配置的 `<remote>/<branch>`（优先已有 `refs/remotes/…`）→ 本地 `refs/heads/<upstream>`（写入 `remote=.`）→ `the requested upstream branch '…' does not exist`（129 / `LBR-CLI-003`）。自身 upstream 只警告、不写配置。`branch -vv` 与 `status` 对 `remote=.` 显示短名（`[main]` / `...main`），不显示 `./main`。Git 对 P6 形态用 128，Libra 保持 129（ADR-HF-02）。
- `#477` HF-08：创建时 `-t/--track[=direct|inherit]` 与 `--no-track` 复用 HF-07 解析。`--track` / `-t` 为 `direct`；`--track=inherit` 复制起点本地分支的 upstream，起点不是分支则 `cannot set up tracking information; starting point '…' is not a branch`（129 / `LBR-CLI-003`，Git 128）且不创建。与 `-d`/`-m`/`--list` 或单独 `--track` 时忽略该标志。`--no-track` 创建但不写 tracking。`require_equals` 避免 `-t t1` 把分支名当成 mode。
- `#477` HF-09：默认列表按 refname（本地在前、远程在后），当前分支不再置顶；`--format` / `--json` 共用该顺序。`-v`/`-vv` 按最长显示名（含分离 HEAD 行）对齐名称列。分离 HEAD 首行为 `* (HEAD detached at <abbrev7>)`。`-a` 远程名为 `remotes/<remote>/<branch>`，`-r` 省略前缀。`--sort` / `branch.sort` / `--ignore-case` 行为不变。
- `#477` HF-25：`branch -d` 三类拒绝（未完全合并、不存在、当前工作树检出）用 `with_exit_code(1)`，稳定码仍为 `LBR-REPO-003` / `LBR-CLI-003`；`LIBRA_FINE_EXIT_CODES=1` 不能覆盖。未完全合并文案为 `the branch '…' is not fully merged`。
- 公开参数/子命令包括：`[<new_branch>] [<commit_hash>]`、`-t, --track[=direct|inherit]`、`--no-track`、`-l, --list`、`-d, --delete <DELETE_SAFE>`、`-D, --delete-force <DELETE>`、`-u, --set-upstream-to <UPSTREAM>`、`--unset-upstream [<BRANCH>]`、`--show-current`、`-m, --move <OLD_BRANCH> <NEW_BRANCH>`、`-r, --remotes`、`-a, --all`、`--contains [<commit>]`、`--no-contains [<commit>]`、`--points-at <object>`、`--merged [<commit>]`、`--no-merged [<commit>]`、`--sort <key>`、`-c, --copy <OLD> <NEW>`、`-C, --copy-force <OLD> <NEW>`、`--ignore-case`、`--format <format>`、`--column[=<MODE>]`、`--no-column`、`-v, --verbose`（`ArgAction::Count`）。`--format=<fmt>`（query 组）在 List 人类输出下经 `for_each_ref::render_ref_format_lines` 渲染（取代 `* name`/`-v`/`--column`，JSON/quiet 不受影响）。`-v`/`--verbose`（verbose>=1）在 List 人类输出每行追加 ` <短sha> <subject>`（`branch_verbose_suffix` 经 `parse_commit_msg` 取首行 subject）；`-vv`（verbose>=2）在 sha 与 subject 之间额外插入上游段 ` [<upstream>: ahead N, behind M]`（`branch_upstream_segment` 读 `branch.<n>.remote`/`.merge` → `refs/remotes/<remote>/<merge-short>` 经 `get_target_commit` 解析 + `status::upstream_ahead_behind`（共用 `internal::merge_base::ahead_behind`）；remote-tracking ref 未 fetch 或计数不可得（后者发 warning）时省略计数仅显示 `[<upstream>]`，无上游时不插入）；render_branch_output/branch_verbose_suffix 为此改为 async；非列表动作与 JSON/quiet 不受影响；`-v` 优先于 `--column`（后者为纯名列布局）。`--column`（`always`/`auto`/`never`，bare 即 `always`；模式经 `tag::resolve_column_enabled` 校验、宽度经 `column_layout_width`）在 List 人类输出里用 `format_branch_columns` 把条目（current 分支带 `*` 前缀、纯名无颜色以保证列宽计算）按 column-major 排布；JSON/quiet 与非列表动作不受影响。`--no-column`（= `--column=never`）经 clap `overrides_with` 撤销先前的 `--column`（last-wins，读 `column` 字段，`no_column` 不直接读取），分支默认每行一个故单独为 no-op。`-c`/`-C`（`copy_branch_impl`）在 `<old>` 的提交处创建 `<new>` 并复制上游配置（`branch.<old>.remote`/`.merge`→`<new>`），保留源分支、不移动 HEAD；`-c` 在目标已存在时报 `AlreadyExists`，`-C` 覆盖。一参数形式复制当前分支。`--merged`/`--no-merged`（缺省 HEAD）复用 `log::get_reachable_commits` 计算目标可达集合，保留（或排除）tip 在该集合内（即已合并入目标）的分支，是 `--contains` 的反方向。`--sort <key>`（`refname`/`version:refname`/`v:refname`，加 `committerdate`/`creatordate`/`authordate` 日期键、`objectsize` 与 `objectname`，前导 `-` 反转）在 `collect_branch_output` 内由 `sort_branch_entries` 排序条目（人类与 JSON 输出一致，且不再按 current-first 排），未知 key 报 `LBR-CLI-002`；dash-leading 值需用 `--sort=-refname`。日期键：`sort_branch_entries` 先用 `ObjectHash::from_str`+`load_object::<Commit>` 把各 tip-commit hash 预载成 `HashMap<hash,i64>`（→`committer.timestamp`；加载失败→0），再按时间戳升序排序、refname tiebreak。
...
| ✅ 已实现 | 详细列表 `-v` / `-vv` / `--verbose` | `branch_verbose_suffix` 在 List 输出追加 ` <短sha> <subject>`；`-vv` 经 `branch_upstream_segment` 额外插入上游 tracking `[<upstream>: ahead N, behind M]`（经 `status::upstream_ahead_behind` 调用与 merge-base 共用的 `internal::merge_base::ahead_behind`；计数不可得时只显示 `[<upstream>]` 并发 warning，#486）。带集成测试（`branch_verbose_shows_sha_and_subject` + `branch_vv_shows_upstream_segment` + `branch_vv_counts_upstream_shapes`）。 |
| ✅ 已实现 | 本地 upstream 与 `-u <upstream> [<branch>]` | `resolve_upstream_spec`：远程名+tracking 优先，否则本地分支写 `remote=.`；自身 upstream 警告零写入；`branch -vv` / `status` 显示短名。带单测（`resolve_upstream_spec_*` / `set_upstream_impl_warns_on_self_upstream`）与子进程矩阵 `test_branch_set_upstream_to_with_branch_arg_and_local_upstream_matrix`。 |
| ✅ 已实现 | 创建时 `--track[=direct|inherit]` / `--no-track` | `create_branch_with_optional_track`：先规划 tracking 再创建，哈希起点 T6a 零写入拒绝；inherit 复制起点 `branch.*`；与 list/delete/rename 组合忽略。矩阵 `test_branch_create_track_modes_matrix`。 |
| ✅ 已实现 | 默认列表 refname 排序与 `-v` 对齐 | `collect_branch_output` 在无 `--sort`/`branch.sort` 时按 `plain_name` 排并 `sorted=true`；`-a` 远程 `remotes/` 前缀；renderer 不再 current-first，`-v` 用 `list_name_column_width` 对齐，分离 HEAD 为 `* (HEAD detached at <7>)`。矩阵 `test_branch_list_refname_order_and_verbose_alignment_matrix`。 |
| ✅ 已实现 | `branch -d` 拒绝 exit 1 | `DeleteNotFound` / `DeleteCurrent` / `NotFullyMerged` 经 `with_exit_code(1)`；稳定码不变。矩阵 `test_branch_delete_refusal_exit_codes_matrix`。 |
