...
- external conclusion of a stopped sequence item by commit（#477 HF-29，ADR-HF-03 第 2、4、5 条）：非 `--dry-run` 的 `commit` 在 HEAD 更新成功后复用 HF-01 的 `conclude_stopped_cherry_pick` / `conclude_stopped_revert`。快照取自写入之前，因此提交完成后才开始的序列不会被收尾。待办为空时清除状态，否则写入 `stop_concluded`。收尾失败不回滚提交，只追加 warning（exit 0）。`--dry-run` / `--porcelain` 不读不写序列状态。
- commit 结束进行中的 merge（#477 HF-27，ADR-HF-21）：非 `--dry-run` 的 `commit` 在 merge 进行中时父提交为 HEAD 加 `MergeState` 记录的对方提交（`merge::merge_commit_parents`），消息默认取 `merge::merge_commit_message`（冲突态附 `# Conflicts:` 段；`--no-edit` 用 `CleanupMode::Verbatim` 保留注释，编辑器模式剥离；`-m`/`-F` 覆盖）。HEAD 更新后经 `merge::conclude_merge_after_commit` 清除 `merge-state.json` 再应用持有的 autostash。merge 进行中 `--amend` 映射 `CommitError::AmendDuringMerge`（128 / `LBR-REPO-003`）。`--dry-run` 设 `dry_run_preview`，不打印伪 hash、不写状态。`merge --squash` 写入 `.libra/SQUASH_MSG`，随后 `commit` 单父预填该消息并在成功后 `clear_squash_message`。部分提交保持 129。
- NothingToCommit 分型（#477 HF-05，ADR-HF-06）：空暂存区按 `status::changes_to_be_staged_split_safe` 分成三种拒绝——干净工作树 `nothing to commit, working tree clean`；仅未跟踪 `nothing added to commit but untracked files present (use "libra add" to track)`；有未暂存已跟踪改动 `no changes added to commit (use "libra add" and/or "libra commit -a")`（未暂存优先于仅未跟踪）。`-s` 不改变拒绝。从未跟踪过文件仍是 `NothingToCommitNoTracked`。消息清理后先判空消息（纯注释 `-t` 模板 → `EmptyMessage`），再判模板未编辑（`TemplateUnedited`）。退出码 128 / `LBR-REPO-003` 与 JSON `error_code` 不变。
- `--allow-empty-message`（#477 HF-06，ADR-HF-07）：在空消息检查与模板未编辑检查之前短路，两者都绕过；不隐含 `--allow-empty`。`persist_and_run_commit_msg_hook` 对空文件同样放行，但 hook 自身仍可拒绝。
