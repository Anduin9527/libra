...
- 源码分层：主要实现文件为 `src/command/status.rs`。参数/子命令类型包括：`StatusArgs`、`PorcelainVersion`、`UntrackedFiles`；输出、错误或状态类型包括：`StatusData`（所有渲染器共享的中心数据结构，承载 staged/unstaged/unmerged/ignored/stash/upstream/merge_state/porcelain_v2 字段，模块内可见）、`Changes`、`StatusError`、`UpstreamInfo`、`MergeStatusInfo`；核心数据函数为 `collect_status_data`，辅助执行函数包括：`execute`、`execute_safe`、`execute_to`、`changes_to_be_committed_safe`、`changes_to_be_staged_split_safe`。upstream ahead/behind 计数由 `resolve_upstream_info` 经 `upstream_ahead_behind`（`branch -vv` 共用）调用 `internal::merge_base::ahead_behind`：与 merge-base 共用标记遍历，读取 shallow 边界并视为根；计数不可得时 `UpstreamInfo.ahead/behind` 为 `None`，并向本次调用的 `warnings` 追加 `upstream_counts_unavailable`（source `metadata`，不走进程级 `emit_warning`，且不计入 `rename_scan_blocked`；`branch -vv` 仍经 `emit_warning`），短格式不带括号、porcelain v2 省略 `# branch.ab`、JSON 为 `null`（#486）。
...
- 2026-07-23（plan-20260714 §B.4.3，R0-4 首片，v0.19.49）：新增 pre-clap `normalize_status_argv`（cli.rs 重写链末端接入）——仅当 clap 全局参数 metadata 定位出的首个子命令为 `status`/`st` 时改写其参数区：`--find-renames[=RAW]` 记入 occurrence 列表并以 `=50` 占位让 Git raw 语法（裸整数=0.N、`N%`、小数）过 clap,`--renames`/`--no-renames` 同列;`resolve_status_threshold` 只按**末位** occurrence 取值（真三向 last-wins,clap 的 pairwise overrides 不再决定语义,自覆盖 attr 仅保证可重复）。`-z` 增加 `--null` 别名并对 `--long`/`--cached`/`--check-dirty` 声明冲突;裸 `-z`/`--null` 无显式格式（含 config short）时在 resolver 层强制 porcelain v1。公开 `execute_safe` 委托 `execute_safe_with_resolution(…, None)`（pub(crate)）;API/struct-literal 路径保留 0–100 百分比（文档已记录 CLI 更完整）。非 status 子命令 argv 原样返回（`diff status --find-renames=505` 等 pathspec 场景零改写）。回归:`bare_z_emits_porcelain_v1`、`st_bare_z_emits_porcelain_v1`、`bare_null_emits_porcelain_v1`、`find_renames_raw_grammar_and_last_wins`（含非法末位 fail-closed 与非法被后值覆盖）、`normalize_ignores_diff_pathspec_status`;既有 toggle 用例断言更新为 last-wins 语义。剩余 R0-4 子项:cluster 内格式 flag 元数据表、`StatusFormatFlags` 独立结构、`InvocationWarningCtx`、`compat_status_args_parser_compile` 守卫、`--long -z` 组合的 normalize 层用例矩阵。
