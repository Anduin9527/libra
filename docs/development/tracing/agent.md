...
- **scope 三类**：`committed`（TurnEnd / SessionEnd，已实现）、`subagent`（两类独立证据：A0-02 的 Codex `SubagentStart` / `SubagentEnd` **boundary checkpoint** 使用 `parent_checkpoint_id` 指向最近 committed；M5/DR-06 的 Claude `<session>/subagents/*.jsonl` **content checkpoint** 保持 `parent_checkpoint_id=NULL`，版本由 source claim/revision 表表达）、`temporary`（ToolUse 等，仍为规划）。boundary/content 不互相去重；只有 provider 稳定 ID 唯一匹配时才经独立 link association 关联，否则 `unresolved`。`temporary` 的虚线箭头表示规划路径，尚未在当前 hook ingest 中落地。
...
| `codex` | `Codex` / `codex` | first-batch supported；hook-installable | hook install/uninstall/status、lifecycle ingest、transcript read、checkpoint/export、review evidence | 以上游实测为准：当前写入用户级 `$CODEX_HOME/hooks.json` 与 `config.toml` `[hooks.state]` trust 记录；不得假定项目级 `.codex/hooks.json` 是 Libra 新写入目标 |
| `opencode` | `OpenCode` / `opencode` | first-batch supported；hook-installable | hook install/uninstall/status、lifecycle ingest、transcript read、checkpoint/export、review evidence | 以上游实测为准：当前写入 `.opencode/plugin/libra-hooks.js`（单数目录，上游 SDK 规范路径，与本文「OpenCode 安装流程契约」§3 一致）并保留用户配置；复数 `.opencode/plugins/` 仅作为携带 Libra-managed 标记的重复受管文件的检测/清理输入，不作为 Libra 新写入目标 |
...
| 合规性与标准符合性 | ✅ 合理 | retention、erasure、raw export 授权、append-only audit、redaction report、本地删除一致性与 cloud mirror 条件边界均已固化为可执行规格；方案不再把 redaction 误当作完整合规。 | transcript retention、删除权、raw export 授权、audit log 不可变性和 redaction report 已在本文固化；普通 checkpoint retention 已传播 D1 prune fence 并删除远端 capture catalog 行。session erasure tombstone 已传播 D1（plan-20260714 PD-03）；R2 物理删除仍待建。 | 落地 retention / GC、删除一致性矩阵、redaction report、raw `--allow-raw` 显式授权；audit log 必须 append-only，字段包含 actor、scope、raw/detail 标志、redaction summary、object ids、reason；session erasure tombstone 传播已落地（sync 发布 + restore tombstone 优先）；文档不得把 tombstone/catalog 删除误写成 R2 物理删除（R2 payload 删除仍 deferred）。 |

**总体结论**：文档方向、边界和落地门禁均已达到“合理”评级；AG-18/AG-20/AG-22/AG-23/AG-24a/AG-24 已按本文门禁补齐实现证据（schema 版本、env fail-closed、provenance、crash recovery、分页、合规），后续重点是通过测试与文档守卫防止「有意差异」退化为「过时」。
...
| 合规性与标准符合性 | 满足。retention、erasure、raw export、audit、redaction report、本地/cloud 条件边界已成可执行规格。 | GC/erasure 测试证明重写 `refs/libra/traces` + 删除 DB/object_index；raw export 必写 append-only audit；redaction report 不含原文；普通 checkpoint retention 的 D1 prune fence/catalog 删除与 session erasure tombstone 传播（PD-03）均已实现；R2 物理删除仍 deferred，release notes 写明 R2 payload 不物理删除即可。 |
...
3. `agent_session` / `agent_checkpoint` 的 D1 mirror **已默认生效**（`libra cloud sync` upsert，`libra cloud restore` 读回；M5 同步往返 subagent claim/revision/link companion，`agent_usage_stats` 仍无 mirror）。普通 checkpoint retention 已传播耐久 D1 prune tombstone，并从远端 capture catalog 删除 checkpoint/revision/link；session erasure tombstone 传播与 catalog 删除亦已落地。`libra cloud restore` 是**双向 tombstone 优先**：远端 tombstone 与本机 `agent_import_tombstone` 都在 restore 事务内过滤（本机 erase 之后、下一次 sync 之前从旧镜像 restore 也不会复活），schema 触发器为兜底。**仍 deferred 的只有 R2 物理删除**——已擦除内容的 payload 仍存在于 R2，未见过该 tombstone 的另一台机器仍可从镜像取回，因此 erase 不构成跨机不可逆删除。
...
- Synopsis：`libra agent <status|list|enable|add|disable|remove|session|checkpoint|clean|doctor|push|rpc>`。
...
> 本节是 Gate 8 起跑时的事实起点，保留用于解释为什么 AG-18~AG-24a 要求这些门禁。它不是当前实现状态；当前状态以上文「当前状态」、`tests/INDEX.md` 和 §10 进度记录为准。凡下表标「净新建」的项，是当时的拆卡依据；落地后不得再把这些历史差距当作当前未完成项上报。
...
| T6 | Untrusted seed（issue-link / prompt）进入 mutating workflow | review `--fix` / investigate fix | 两命令仅提交编译期固定请求；`ReviewFixInput::UntrustedSeed` 在 runtime discovery 前拒绝 | 默认拒绝；固定 admission 与显式 runtime approval；seed 不跨越 bridge | fail-closed，`LBR-AGENT-011` 提示只读替代 |
...
| T10 | 已擦除 session 被 cloud restore 复活 | local GC / cloud sync / restore | **已缓解（PD-03）**：session erasure tombstone 经 `libra cloud sync` 发布到 D1（fence 门控 UPSERT + 级联删除 session/checkpoint/companion 镜像行），`libra cloud restore` tombstone 优先（擦除的 session 绝不回灌）并把 fence 落回本机；R2 物理删除仍 deferred | 本地 erasure 重写 ref + DB/object_index 删除 + `agent_import_tombstone`；sync `sync_agent_import_tombstones_batch`；restore 过滤 + `persist_local_import_tombstones` | 文档不得把 tombstone/catalog 删除表述成 R2 物理删除；R2 payload 删除仍待建 |
...
- ⚠️ **fix-bridge 前置（plan-20260715 W6-02 重启条件已登记）**：内部 AgentRuntime **serialized mutating 入口已就绪**，锚点：
  ...
  **2026-08-24 DF-09 状态**：`review --fix` 通过 `runtime::fix_bridge` 和
  ...
  `LBR-AGENT-011`。**2026-08-25 DF-04 状态**：`investigate fix` 校验已有 run 后
  复用同一 controller lifecycle 与 runtime helper，提交独立的编译期固定请求；run id、
  topic、stances、findings、attachments 均不进入请求。它不创建第二条 queue、hardening
  表或 approval 表，无 runtime 时同样保持 `LBR-AGENT-010`。
...
| `Launcher` / `SkillDiscoverer` / `SessionBaseDirProvider` | read-only review/investigate launcher 已落地；skill event extraction 已落地；独立 `SkillDiscoverer` / `SessionBaseDirProvider` trait 未公开 | agent 启动器、skill 发现、会话基础目录 |
...
| 标准化 `EventType`：SessionStart/TurnStart/TurnEnd/Compaction/SessionEnd/**SubagentStart/SubagentEnd**/ModelUpdate/ToolUse（9 个） | `LifecycleEventKind` 已有 13 个变体并包含 `SubagentStart`/`SubagentEnd`；Codex provider parser/settings 已映射原生 subagent hooks；**A0-02 已接通** `CheckpointScope::Subagent`：每个 subagent 边界物化独立 checkpoint（`parent_checkpoint_id` 链回最近 committed） | 已补 subagent 两事件与独立 `scope='subagent'` checkpoint；后续仅剩 temporary-scope 等扩展 |
...
| entire CLI | libra 当前 | 差距说明 |
|---|---|---|
| `SkillDiscoverer` + `skilldiscovery` 注册表：按 agent 提供 curated review skills（`claude-code`→`/review`,`/security-review`,`/simplify`；`codex`→`/review`）+ install hints | `skills/` 有通用 skill 系统（loader/parser/dispatcher/scanner），**无 agent-specific discovery** | 外部 agent 无法被发现/推荐专属 skill；`SkillEvent`（E7）extraction 已落地（进入 checkpoint 提取 metadata），但**未纳入 searchable session/checkpoint/search 投影** |
...
- 实现 `AgentInvestigateWorkflow`：**E8-entire** strict round-robin + **E8-libra** `state.json`/`manifest.json` 持久化 run id/topic/agents/quorum/max_turns/next_agent_idx/pending_turn/stances/findings_doc/starting_sha；支持 continue/show/clean。DF-04 已让 `investigate fix` 校验 run 后复用 AG-22 的同一受控 bridge，并只提交不含 run 内容的固定请求；untrusted seed 进入 mutation 时返回 `ERR_AGENT_UNTRUSTED_SEED_FOR_MUTATION` 对应的真实编号。
...
- investigate 测试覆盖 max-turns、quorum reached、no-new-findings stalled、agent failure pause、continue resume，以及共享 bridge 的 fixed-admission、repair/no-patch 和 no-runtime fallback。
- read-only review/investigate 不依赖完整内部 AgentRuntime / Web-only 迁移；`review --fix` 与 `investigate fix` 的 mutating tool 调用均经过同一 AgentRuntime serialized queue、approval/sandbox/tool gate。任何新增 mutating 入口未就绪时，测试必须断言 actionable unsupported，而不是跳过或假成功。
...
| `ERR_AGENT_FIX_BRIDGE_UNAVAILABLE` | `review --fix` 或 `investigate fix` 没有活跃获授权 Code runtime | 128 | 明确启动 `libra code --control write` 的恢复动作 | AG-22/AG-23 fail-closed tests |
...
libra investigate fix <run_id>   # 复用 review 的受控 bridge；只提交固定请求，不转发 run 内容
```
...
| cloud R2 physical deletion | deferred（session-erasure tombstone 传播已随 plan-20260714 PD-03 生效：sync 发布 + 级联删除 D1 镜像行、restore tombstone 优先且落回本机 fence） | R2 payload 在擦除后仍物理存在于 durable tier | release notes 写明 R2 payload 不物理删除；tombstone/catalog 面已闭环 |
...
#### 0. 历史起点 → 目标对照（source-grounded）

> 本表保留 2026-06-17/07-04 起跑时的差距判断，用于解释 AG-16~AG-24a 的拆卡来源。当前实现是否完成，以本文件「当前状态」、测试矩阵、`tests/INDEX.md` 与 `plan.md` §10 为准；不要把本表的历史「缺口」列当作当前未完成项。

| 轴 | 历史起点锚点 | 起跑时缺口 | 目标 / 当前 closeout 口径 | AG |
|---|---|---|---|---|
...
| hook 注册表 | `find_provider(name)`（`providers/mod.rs`，仅 `"claude"`/`"gemini"`） | 字符串键、缺 codex/opencode | 当前 `providers/{claude,codex,opencode}` 已落地 first-batch provider；Gemini 仅 legacy compatibility | AG-19 |
...
| lifecycle 事件 | `LifecycleEventKind` 13 变体（含 `SubagentStart`/`SubagentEnd`）+ `#[non_exhaustive]` | — | **A0-02 已落地**：subagent events 进入 metadata，且 `CheckpointScope::Subagent` 在边界物化独立 checkpoint | AG-19 / A0-02 |
| roster | `STABLE_AGENT_SLUGS`=`claude-code`/`gemini`（`mod.rs:216`） | 与第一批 `claude-code`/`codex`/`opencode` 不一致 | 当前删除独立常量，CLI roster 从 registry `supported_slugs()` 派生；Codex/OpenCode HookProvider 已落地，Gemini unsupported + uninstall-only | AG-17/AG-19 |
| external 安全 | `RpcAgent::spawn`（`rpc.rs:162`）`Stdio::inherit()` 且**未 `env_clear`** | 子进程继承 `*_API_KEY`/`LIBRA_STORAGE_*`/`LIBRA_D1_*` | 当前 `env_clear()` + allowlist、stderr capture/redact、settings gate、provenance、内置 slug 仿冒防护已落地；fd-exec 仍延期 | AG-18 |
| redaction | `RedactedSink`（`redaction.rs:219`）Phase-1 placeholder，未接 writer | 任何新持久化路径可绕过 redaction | 当前 hook ingest/checkpoint/export 路径有 redaction-before-persist 与 raw `--allow-raw` audit；未来新增 uploader/writer 仍必须保持类型/测试门禁 | AG-19/AG-20/AG-24a |
| checkpoint 幂等 | `agent_checkpoint` 纯 INSERT（`runtime.rs:900`，无 UPSERT） | 重放/崩溃重试主键冲突；窗口 B | 当前 catalog insert idempotency、doctor repair、prune window guard 和 reader/export tests 已落地 | AG-20 |
...
pub trait SubagentAwareExtractor: ObservedAgent { /* extract_parent_and_subagents(parent, child_sources)：modified-files 并集 + parent/child usage 分型 */ }
...
> **2026-07-06 closeout 复核**：AG-16~AG-24a 的 deterministic test targets 均已落地并在 `tests/INDEX.md` 登记。注意注册口径不同：`tests/compat/*` 文件必须在 `Cargo.toml [[test]]` 显式登记；`tests/*.rs` 顶层 integration target（如 `agent_lifecycle_event_test`、`agent_review_workflow_test`）由 Cargo 自动发现，不需要 `[[test]]`。`tests/command/agent_checkpoint_test.rs` 已在 `tests/command/mod.rs` 中 re-export，进入 `command_test`。
>
...
| AG-24 partial（已交付） | `compat_agent_docs_contract`：已注册 | `compat_agent_docs_contract::agent_doc_keeps_claudecode_marked_removed_not_active` | 守卫 `claudecode` 不得复活 + 文档事实一致性 |
| AG-24a/AG-24 docs/schema/compliance/source-of-truth | `compat_agent_docs_contract`：已注册 | `compat_agent_docs_contract::agent_doc_tracks_schema_versioning_and_retention_policy`; `compat_agent_docs_contract::agent_doc_tracks_code_agent_runtime_source_of_truth` | 守卫 public schema/retention/raw export 约束与旧事实源链接不复活；新增 public schema 时同步该 target、`tests/INDEX.md` 和 `tests/compat/README.md` |
...
只有触达 `src/internal/ai/observed_agents/*`、`src/command/agent/*`、external `libra-agent-*` RPC、agent checkpoint/export、review/investigate workflow、或 external-agent docs/tests 时才必跑。当前命令必须区分 deterministic targets 与 local-only ignored smoke：
...
cargo test --test agent_checkpoint_reader_test
cargo test --test agent_doctor_repair_test
...
都计费；不信任打开前 metadata，授权后增长也计入）；若 child discovery 在返回可信
字节数前失败，则保守计入该 child 剩余的全部每源 allowance，避免失败重试绕过 batch
cap；deadline 在 discovery 前
...
owner/generation 可以变更或退役 marker。候选结束后不仅等待 background
`object_index` pending 归零，还比较当前 CLI 调用专属的 terminal-failure counter；
队列消息在 enqueue 时固定归属，超过 60 秒预算后才完成也不会污染下一次嵌入式调用。
超时或更新失败
会保留 repair-pending marker，并仅以精确 identity_id+fence_token 把该次 committed
identity 降为 partial；进程崩溃保留 active generation，lease 过期后的 takeover
必须先完成修复。下一次 replay 在允许 coverage no-op 前，先由绝对
deadline 可杀的私有 helper 前台遍历该 session 的 E4 manifest/tree；helper 先取得
SQLite writer slot，在同一事务复核 marker owner/generation、session ownership 与
erase tombstone，再按 writer 的 type/size 语义幂等修复完整 object set。该事务与
session erase 的 catalog/index 删除串行化，禁止 erase 后重新插入 cloud-eligible 行；
无法有界修复时 fail-closed 保留 marker，并提示 `libra agent doctor --repair`。
session erase 的最终 catalog 事务清理该 marker。
...
Claude capture 在 live `Stop`/`SessionEnd` 与历史 import 的父会话写入旁路扫描
`<project>/<provider-session>/subagents/*.jsonl`。项目根、session 目录和 subagents
目录逐 component 以 `O_NOFOLLOW` 打开；枚举期间持有目录 fd，每个 JSONL 再通过
`openat` 相对同一 fd 打开，任何 symlink/non-regular source 都 fail-closed。目录项数
最多 2,048，单次 live capture 最多接纳 16 个 child source，并让整组原始 child
...
raw line/unknown field 不进入对象、digest 或 metadata。父 checkpoint 的 extraction 使用
实际 child bytes 扩展 `SubagentAwareExtractor`：modified-files 去重并集，child usage 只从
child transcript 加总。父 transcript 中出现 `Task` 不再把父 session 总量伪标为
`subagent_usage`；没有可归属 child source 时只标 partial/warning。

迁移 `2026071406` 引入三层关系：
...
- `agent_subagent_link` 把 content checkpoint 与可空 boundary checkpoint 分开保存。
...
对象产生后事务失败只允许留下不可达对象和 `cleanup_pending` marker；catalog/revision/
current leaf/ref 不得部分可见。prune 可回退 current leaf，但必须递增 claim
`sync_revision`，且不得回退 `revision_cursor` 或复用修订号。down migration 在存在
revision high-water、reservation 或已推进 session generation 时拒绝回滚。session erase
通过外键级联 claim/revision/link；doctor 对 current
`unresolved_subagent_link` 报 `manual_required`，不猜测、不自动修复。
cloud sync/restore 按 session → checkpoint → revision → link → claim 顺序镜像这三张
companion 表（claim 最后发布，确保其全部依赖已存在）；claim 的 transient
owner/lease/attempt 不上传，restore 只恢复 idle claim，
但保留空 current leaf 的 `revision_cursor` high-water。session、checkpoint、claim 与 link
使用独立的单调 `sync_revision`，不再把本地 wall clock 当作冲突代次；source revision
保持 immutable。prune 重建或已验证 doctor repair 时只允许在 checkpoint 其它 identity 字段
不变的前提下推进 `tree_oid`/`metadata_blob_oid`/`traces_commit` 和 checkpoint generation；旧 clone 不能写回
重建前的行。同一 generation 或 immutable identity 内容分歧时 restore 整体回滚。远端
`agent_capture_generation` manifest 以
`publishing`/`complete` 状态和 writer token 把五类批次 fenced 为一个可恢复发布：中断后
下次 sync 可 takeover，restore 只接受前后两次读取均为同一 complete generation 的快照。
manifest 绑定 fenced traces head、object-index catalog generation 与 checkpoint 可达
object-index 投影的 canonical digest/count；capture 阶段对所需 OID 做有界查询，不扫描/下载
无关的大仓库历史。普通 checkpoint retention 在本地 prune 事务写耐久 tombstone，sync 先发布
fence 再删除远端 checkpoint/revision/link，D1 拒绝旧 clone 重建该 identity；session erase 不写
该 fence。restore 在下载对象或应用通用 refs metadata 前，用稳定的 completed generation 检查普通
prune tombstone；通用 metadata 跳过 generation-fenced traces ref。远端 completed 代次仍含已在
本地普通裁剪的 checkpoint 时 fail-closed 并要求先 sync，不复活其对象、ref、catalog 或 companion。
若 deferred erase 使 D1 留有本地缺失且无 fence 的 checkpoint，sync 在开始新
generation 前失败并保留上一 completed 快照。当前客户端只读写 v2 远端 session/checkpoint 表，首次把 legacy session/checkpoint
以 generation 0 收养，清理旧版 best-effort mirror 留下的无 session checkpoint orphan，
再安装 trigger 拒绝旧版无 fence 写入。显式恢复本地已擦除 identity 时，持久化的 capture
incarnation 会让 session generation 严格前进，并为新 child source 派生新的 opaque namespace，
避免复用 D1 保留的旧 revision key；这不等同于删除远端旧 capture。restore 在五类 capture catalog
表和 fenced object-index 投影之间共享 100,000 行聚合预算；完整 object-index 分页使用 keyset cursor，
并在前后校验 trigger 维护的 catalog generation，变化时最多重试三次。
sync 在完成 manifest 前对已有 R2 payload 重算内容 hash；缺失或损坏对象由已验证本地对象覆盖并回读。
restore 在同一 manifest fence 内读取 object index，已有本地对象须
解码并重算 hash，损坏对象会从 R2 原子重下；完整 traces 链和全部 checkpoint tree/blob
验证通过前不得写入 catalog。检测到代次变化最多重试三次。每次 D1 请求最多 30 秒，agent-capture sync/restore 总计
最多 120 秒；发布完成前还会复读全部远端关系并验证 session 依赖、revision/current/link
一致性，以及每个 checkpoint 的 ref 路径和全部可达对象都已上传。旧版没有 manifest 且
存在 capture 行的远端备份须先由当前客户端执行一次 `libra cloud sync` 才能 restore；
没有 capture 行的旧备份按纯 Git 备份跳过该层。restore 只读探测远端 schema，不安装
legacy writer barrier、不执行 generation-0 adoption。任一 companion 行
上传/恢复失败均进入显式失败计数并使 agent-capture mirror 步骤失败；machine JSON 模式
不会先输出成功 envelope 再返回失败。
...
discovery/import/export，不写 ref/object/catalog/claim/tombstone。

JSON v1 固定为 `{schema_version,state,session,turns,subagents}`。indexed turn 按
`logical_turn_key`/coverage schema 排序，ordinal 为返回数组中的零基派生序号；revision 历史
append-only 展示。共享 whole-transcript checkpoint 可同时支撑多个 turn，任何单 turn 升级
都不会隐藏它。没有已提交 coverage revision 的旧 session 以 `unindexed` checkpoint
chronology 呈现，不回填。subagent content 节点原样显示 `resolved`/`unresolved`。命中本地
tombstone 时返回 `erased` 空图且不重建 session；session 与 tombstone 均不存在时返回
`LBR-AGENT-021`。机器 schema、隐私列白名单、共享 checkpoint、legacy、erased、`--repo`
preflight 与运行前后 capture/import/export 表零变化均由 `agent_graph_test` 钉死。
...
| 命令面 | `--local-dev` / `--force` for `agent add/remove` | `list` / `add` / `remove` 已落地并保持 canonical `status` / `enable` / `disable`；`--local-dev` / `--force` 未公开，且不得出现在用户示例中。若未来实现，必须同时挂在 canonical 与 alias 上。 | entire `agent_group.go` / AG-17 deferred parity |
...
| 数据模型 | cloud mirror tombstone propagation for agent capture data | D1/R2 agent capture mirror **已实现并默认生效**：`libra cloud sync` 每次把 `agent_session`/`agent_checkpoint` upsert 到 D1，`libra cloud restore` 经 `restore_agent_capture_from_d1` 读回；普通 checkpoint retention 传播 D1 prune tombstone 并删除远端 capture catalog 行。**session erasure tombstone 传播已随 plan-20260714 PD-03 落地**：`erase_session_local` 写的 `agent_import_tombstone` 由 `libra cloud sync` 经 `sync_agent_import_tombstones_batch` 发布到 D1（generation fence 门控 UPSERT，`erased_at` 取 MAX 幂等）并在同 fence 内级联删除该 session 的 mirror 行（claim→revision→link→checkpoint→session）；`libra cloud restore` tombstone 优先——擦除的 session 绝不回灌，且经 `persist_local_import_tombstones` 把 fence 落回本机；旧远端无 tombstone 表时 restore 保持只读容忍。**仍 deferred 的仅剩 R2 物理删除**（payload 在 durable tier 物理存在）。**A0-10 守卫已随实现翻转**：`compat_agent_docs_contract::agent_doc_declares_cloud_tombstone_deferred` 钉住「session tombstone 传播已生效 + R2 物理删除 deferred」新事实；`command_test::agent_erasure_local_tombstone` 断言本地 tombstone 携带传播所需字段且幂等；L3 `agent_cloud_tombstone_test` 在真实 D1 上证明发布后 mirror 行被移除、restore 目录携带 tombstone 且重复发布幂等。 | AG-24a / M4 local barrier + PD-03 cloud tombstone |
| 多 Agent | `review/investigate --checkpoint <id>` scoped review（review 目标物化） | **A0-06 已实现** findings 对象化 + manual attach：`findings_oid` 在 finalize 写入内容寻址 blob（`agent_findings` object_index tag，doctor 可修复），`review/investigate attach <run_id> <file>` 命令面把外部文件脱敏后对象化并记入 `manual_attach`。**scoped review 已实现（plan-20260714 PD-02）**：`review --checkpoint <id>` / `investigate start --checkpoint <id>` 把 checkpoint 的整个 inner tree（metadata/manifest/transcript）以只读文件物化到 `<run_dir>/checkpoint-input/`（`internal::ai::checkpoint_input`，单文件 64 MiB / 总量 256 MiB、路径消毒），并把该目录作为 reviewer/investigator 的**全部** workspace——不物化任何 worktree 快照、不伪装 worktree diff；scoped prompt 声明内容为捕获 transcript（不可信数据）。命令层 `resolve_checkpoint_input_spec` 在创建任何 run 之前 fail-closed 验证（id 不存在/树非法/blob 不在本地对象库 → 无 run 残留）；investigate 把 spec 持久化进 `state.json`（`checkpoint_input`），`continue` 恢复只会重物化同一 checkpoint；物化产物位于 run 目录内，与 run 共享 retention（`clean` 一并移除）与 orphan release（`workspace_root`），doctor 无新增孤儿类别。守卫：`agent_review_workflow_test::review_checkpoint_scoped`、`agent_investigate_workflow_test::investigate_checkpoint_scoped`。 | E8-libra / A0-06 + PD-02 scoped-review materialization |
...
|---|---|---|---|---|---|
| 普通 retention/prune 删除单个 checkpoint | 重写 `refs/libra/traces` 剔除该 checkpoint | 删除该 checkpoint 的本地 `object_index` 行 | **已实现**：同步耐久 prune tombstone，并删除远端 checkpoint/revision/link/claim leaf；旧 clone 不能重建该 identity | **deferred**：D1 全局 object-index 行与 R2 payload 的物理删除 | 完成新 generation 后 restore 不会复活该 checkpoint；远端内容寻址 payload 仍等待仓库级安全 GC |
| 删除整个 session | 同上 + 删 `agent_session` + 删 session 目录 | 删相关 OIDs | **已实现（PD-03）**：`cloud sync` 发布 `agent_import_tombstone` 到 D1 并在同 generation fence 内级联删除该 session 的 mirror 行（claim→revision→link→checkpoint→session）；usage 行仍无 mirror | **deferred**：R2 payload 物理删除 | 本地 erasure 立即一致；D1 mirror 行随下一次 sync 删除，`cloud restore` tombstone 优先且把 fence 落回本机——擦除的 session 不再回灌；R2 payload 物理存在直至 R2 删除落地 |
...
| `user_id` / `user_name` | TEXT | 执行用户标识。**实现现状（2026-07-08）**：仅从 `GIT_COMMITTER_*` / `GIT_AUTHOR_*` / `EMAIL` / `LIBRA_COMMITTER_*` 环境变量解析（`command/agent/checkpoint.rs`），**当前不回退 repo config `user.name`/`user.email`**；无相关 env 时记为 NULL。仅靠 `libra config` 配置身份、未导出 committer env 的仓库会记录空 actor（后续可补 config 回退切片）。 |
