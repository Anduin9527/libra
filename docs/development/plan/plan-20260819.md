...
| 事实 | 证据 |
|---|---|
| `memory.md` 已补齐 Episode payload、自动信任门、稳定 Cell 与共享 selection receipt 合同 | `docs/development/tracing/memory.md:504-633,1380-1407`、`docs/development/gap/mainline.md` ML-05 receipt 条目（2026-08-27 复核现 `:280`，原稿 `IntentContextSelectionReceiptV1` 已按 ADR-M2-10 标注「已替代」）与 §「已替代」存档段（现 `:769` 起） |
...
**Current evidence:** `ContextFrame` 可含 raw content/attachment，不能扩成 receipt；`docs/development/gap/mainline.md` ML-05「编辑前先读相关 sealed intent/decision」段（2026-08-27 复核现 `:277-284`，其中原稿 `IntentContextSelectionReceiptV1` 已按 ADR-M2-10 标注为「已替代」）。Memory 草案已有 local-only receipt 方向但表名与 mainline 分叉；`memory.md:981-1005,1334-1356`。
...
| `ContextFrame` 可含 raw content/attachment，mainline 已要求另建 versioned sibling receipt | `docs/development/gap/mainline.md` ML-05「编辑前先读相关 sealed intent/decision」段（2026-08-27 复核现 `:277-284`，其中原稿 `IntentContextSelectionReceiptV1` 已按 ADR-M2-10 标注为「已替代」） |
