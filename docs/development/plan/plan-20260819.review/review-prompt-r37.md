...
## R30-r22 附加要求（第二十二次複審；本輪評審期間凍結計劃檔）

本輪是 r21 修訂後的複審。r21 結果（同一凍結版 sha256 `284eb643…`）：Codex `0×P0 + 1×P1 + 3×P2`、Claude `0×P0 + 2×P1 + 7×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r21 行。**被審版本 SHA-256 見 log 檔頭；本輪期間計劃檔不再變更。** 重點：

1. **proposal 下沉**（Claude r21 P1-1）：M2-08 AC3/AC5 是否含 proposal 維度；指紋矩陣 VER 是否已更名並含三態；測試矩陣與故障矩陣是否同步。
2. **identity 簽發面**（Claude r21 P1-2）：`libra memory identity register|revoke|list` 是否定義於 M2-13（命令面、AC、VER、寫集、`prod-files`、EX-M2-02 分子 44/13、審計表）；ADR-M2-05/12 的 CLI proposal 保證是否與之相容；六態 E2E 是否一致。
3. **M2-05 prod-files**（Codex r21 P1）：條件分支是否已定案（複用既有錯誤碼，17 檔）且五處一致。
4. `EX-M2-06/EX-M2-07` 塊是否在 M2-16C 全部 9 條 VER **之後**；`EX-M2-09` 是否逐條枚舉 AC1–AC8；M2-02 A1 是否具名 identity 六列且 VER=9；`R30-r16` 行格式；review log 是否無 `r26` 筆誤；`plan-status` 是否含 r18。
5. 25 卡逐欄、基線事實/版本面/CI job/migration tip/merge-base `2ebd409…`/158 路徑不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r23 附加要求（第二十三次複審；本輪評審期間凍結計劃檔）

本輪是 r22 修訂後的複審。r22 結果（同一凍結版 sha256 `312b06ef…`）：Codex `0×P0 + 3×P1 + 2×P2`、Claude `0×P0 + 6×P1 + 8×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r22 行。**被審版本 SHA-256 見 log 檔頭；本輪期間計劃檔不再變更。** 重點：

1. **M2-02**：`VER=9/8@EX-M2-01`（Granularity 與審計表）、判據塊是否 9 條 VER（`memory_compiler_identity_schema` 在列）、EX-M2-01 waiver 行是否註明。
2. **identity 命令族**：M2-13 是否具名 AC（已認證本地 principal、`--principal` 不得自報、一次性 HMAC、`revoke` 冪等、`list` 僅回 HMAC 前綴）、AC7 seam allowlist 是否含 `register/revoke/list`、M2-08 是否交付 `revoke/list` seam、偽造 principal 回歸是否存在；AC 45/VER 15 與 EX-M2-02 拆分（command_test 7/lib 2/memory_episode_test 4/compat 4）五處一致。
3. **M2-05**：寫集是否不再含 `src/utils/error.rs`（17 檔）、AC1 是否定案複用既有碼。
4. **故障矩陣 proposal 行**：是否 4 格且「禁止結果」為真正禁止項；指紋矩陣 VER 名稱含 proposal。
5. **格式回歸**：審計表末行與 `**Verification 新增标记：**` 是否獨立段落（`rg -n '^\|.*\|\*\*'` 零命中）；M2-16C 判據塊是否在全部 VER 後且無多餘空行。
6. `EX-M2-09` 標籤是否 `ACn-m` 連續；`R30-r16`/`GC-M2-15` 排序；review log 是否無 `r27` 類筆誤；`plan-status` r18/r19/r20/r21/r22 完整。
7. 25 卡逐欄、基線事實/版本面/CI job/migration tip/merge-base `2ebd409…`/158 路徑不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r24 附加要求（第二十四次複審；本輪評審期間凍結計劃檔）

本輪是 r23 修訂後的複審。r23 結果（同一凍結版 sha256 `6721458b…`）：Codex `0×P0 + 2×P1 + 4×P2`、Claude `0×P0 + 3×P1 + 5×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r23 行。**被審版本 SHA-256 見 log 檔頭；本輪期間計劃檔不再變更。** 重點：

1. **EX-M2-02 計數單一定案**：全計劃只有一個分子（45 AC / 15 VER）；waiver 行、判據首行、算式（4+3+2+2+7+4+8+1+12+2=45）、V 清單（15 項含 V1e/V1f）、Granularity、審計表、全域口徑全部一致；VER 拆分 `command_test 7 + lib 2 + memory_episode_test 2 + compat 4 = 15`。
2. **審計表末行與 `**Verification 新增标记：**` 之間是否有物理空行**（`sed -n` 顯示空行；機械檢查是否為『末行下一行為空』形式）。
3. **表格格數**：waiver 表（8 列）與依賴表（8 列）所有行不得多於表寬；`r30-port-inventory.md` 是否已標歷史並修正 merge-base pin（`2ebd409…`）、`memory::testing` 與所有權映射。
4. M2-16C VER 清單中無三個連續空行、`EX-M2-07` 塊前有空行；表格單元格內 `|` 是否已轉義（`rg -n '^\|.*\|.*\|.*\|.*\|.*\|'` 抽查）；M2-01 AC8 尾端無多餘 `」`；`EX-M2-09` AC5-⑦ 含 `list`。
5. review log 是否無 `r29` 類筆誤；`plan-status` r18–r23 完整；25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r25 附加要求（第二十五次複審；本輪評審期間凍結計劃檔）

本輪是 r24 修訂後的複審。r24 結果（同一凍結版 sha256 `c06a8ea3…`）：Codex `0×P0 + 2×P1 + 5×P2`、Claude `0×P0 + 3×P1 + 8×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r24 行。**被審版本 SHA-256 見 log 檔頭；本輪期間計劃檔不再變更。** 重點：

1. **`DEP-M2-SRC-01` 行**：是否恰 8 格；第 7 格（證據）含 `test -d`、merge-base=`2ebd409…`、`rev-parse`、158-path 計數與雙評審證據要求；第 8 格（超時與失敗策略）為「clone 不可用即 `Lifecycle=blocked`、不得以 `gh api`/裸 `git` 替代」；整行無裸 `|`。
2. **entity 所有權**：M2-02=`4/11`（7 張投影 entity）、M2-08=`3/10`（job/observer/observation entity）、M2-13=`4/6`（identity entity）、M2-02F=`4/8`（search_doc entity）、M2-06=`3/6`（只讀）；五張卡的寫集、`Files likely touched`、`Granularity prod-files`、審計表逐格一致；M2-02 AC5 是否明寫分工。
3. **表格完整性**：全文所有 markdown 表格每行有效分隔符等於表頭（含轉義 `\|`）；`EX-M2-07` 塊前有空行且標籤為 `AC3a/b/c`；`EX-M2-09` ⑦ 含 `list`；M2-13 V1f 括注；M2-16C deps 傳遞。
4. review log 是否無 `r30` 類筆誤；`plan-status` r18–r24 完整；修訂歷史 r24 行是否與實際落地一致。
5. 25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r26 附加要求（第二十六次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r25 修訂後的複審。r25 結果（同一凍結版 sha256 `a6c7f050…`）：Codex `0×P0 + 2×P1 + 0×P2`、Claude `0×P0 + 4×P1 + 6×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r25 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點：

1. **`DEP-M2-SRC-01`**：來源判據是否改為 `git -C "$SRC_CLONE" rev-parse --is-inside-work-tree` + `rev-parse --verify`（支援 worktree 的 `.git` 檔案）；158-path 命令範圍是否為 `2ebd409… bc1be587e4`。
2. **identity entity 所有權**：`memory_compiler_identity.rs` 是否在 M2-08 寫集（`3/11`）且 M2-13 回 `4/5`；M2-02 AC5 分工句；DAG 方向不再倒置。
3. **`§0.2`/`§0.4`**：`agent_bridge/memory.rs` 是否歸 M2-16C、入口類型歸 M2-07；r09 修訂行是否有更正腳註。
4. **M2-01K**：寫集是否含 `tests/SERIAL_REGISTRY.tsv`/`.config/nextest.toml`；VER 是否 6（含 `compat_serial_registry`）；trigger 是否 T-1/T-4；審計 `6/8`＋`3/6`。
5. **審計表末行與 `(new)` 段落之間的物理空行**（本次以位元組檢查確認）；表格機械列數；`M2-02R` 無連續空行；`GC-M2-15` 措辭與倉庫事實一致；`plan-template.md` 的 web/worker 歷史口徑注記。
6. review log 是否無 `r31` 類筆誤；`plan-status` r18–r25 完整；25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r27 附加要求（第二十七次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r26 修訂後的複審。r26 結果（同一凍結版 plan sha256 `181fdf52…`）：Codex `0×P0 + 1×P1 + 2×P2`、Claude `0×P0 + 4×P1 + 10×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r26 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點（請以**實測命令**核對，不採信修訂歷史自述）：

1. **`DEP-M2-SRC-01`（第二次修訂）**：`rg -n 'is-inside-work-tree|rev-parse --verify|diff --name-only' plan-20260819.md` 是否落在 DEP 行本身；`test -d "$SRC_CLONE/.git"` 是否已移除；158-path 範圍是否 `2ebd409… bc1be587e4`。
2. **registry**：M2-02F 寫集是否含 `src/internal/model/mod.rs`（`prod-files=9`、審計 `4/9`）；M2-08 是否含 registry（`12`、`3/12`）；M2-16C 是否含 `agent_bridge/{mod.rs,authorization.rs}`（`7`、`4/7`）並在 AC3 寫 `ActionClass`。
3. **M2-03**：是否有 `rg` 命令層後門守衛 VER 且 `VER=5/8`、審計 `5/8` 一致。
4. 審計表末行與 `(new)` 段落間物理空行（位元組核對）；全文表格每行有效分隔符 = 表頭；各卡 VER 清單無分段空行；M2-05 V9 內容性守衛；M2-06 殘留括注；M2-13 非表格代碼 span 的 `|`。
5. 模板 `plan-template.md`：`web/`/`worker/` 於 198/339/703 有歷史口徑註記、`:262` 無重複 `sql/**`。
6. review log 輪次筆誤；`plan-status` r18–r26 完整；25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r28 附加要求（第二十八次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r27 修訂後的複審。r27 結果（同一凍結版 plan sha256 `13b83815…`）：Codex `0×P0 + 1×P1 + 1×P2`、Claude `0×P0 + 3×P1 + 7×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r27 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點（請以實測命令核對，不採信修訂歷史自述）：

1. **M2-16C `ActionClass`**：`rg -n 'ActionClass' plan-20260819.md` 是否命中 **AC3 與 VER**（不再只命中修訂歷史）；`memory.recall=Normal`／`memory.episode.record=Dangerous` 是否明寫；`agent_bridge_approval_test::memory_recall_is_normal_and_record_requires_approval` 是否為 VER；寫集含 `authorization.rs` 且 `Files likely touched` 不再寫「首建」；AC 32/VER 10 與 EX-M2-06/07、Granularity、審計表、waiver 行逐處一致。
2. **M2-03 審計列**：是否為 `7/8 | 5/8 | 2/3`（與卡內 `VER=5/8; landing=2; prod-files=3` 一致）。
3. **M2-05 V9**：是否為字面 33 份清單（非 `$(...)`），EX-M2-05 判據規範 V9 描述同步。
4. 全文表格每行有效分隔符 = 表頭；審計表末行與 `(new)` 段落間物理空行；`**Verification:**` 後統一空行；`$SRC_CLONE` 全部加引號；review log 輪次無筆誤；`plan-status` r18–r27 完整。
5. 模板 `:251` 示例是否已改 `opencode-export-linux`（含 `web-check` 歷史註記）；25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r29 附加要求（第二十九次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r28 修訂後的複審。r28 結果（同一凍結版 plan sha256 `f570cc8a…`）：Codex `0×P0 + 2×P1 + 1×P2`、Claude `0×P0 + 4×P1 + 3×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r28 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點（以實測命令核對，不採信修訂歷史自述）：

1. **M2-16C 計數單一口徑**：`rg -n '31/8@EX-M2-07|9/8@EX-M2-06|= 31。|M2-16C=31|9 個 checkbox' plan-20260819.md` 應零命中；EX-M2-06 判據塊應為 **10 門 B0–B9**（B6=approval）且與 10 個 checkbox 1:1；EX-M2-07 算式 `= 32`；寫集/Files 是否含 `tests/agent_bridge_approval_test.rs`。
2. **可複製命令無 `\|`**：`rg -n '`[^`]*\\\|[^`]*`' plan-20260819.md` 應零命中；源盤點命令應為 pipe-free 等價式（pathspec／多 `-e`／slash）；表格機械列數每行 = 表頭。
3. **M2-02 proposal 契約**：A9 與 Security 是否允許 `proposal_canonical_json` 正文並與 AC3/表職責一致；`handle_hmac` 是否列入 A1 與 VER（七列）。
4. M2-03 `axis` 卡內/審計表一致；審計表末行與 `(new)` 段落間物理空行；`**Verification:**` 空行統一；review log 輪次無筆誤；`plan-status` r18–r28 完整。
5. 25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r30 附加要求（第三十次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r29 修訂後的複審。r29 結果（同一凍結版 plan sha256 `fa65abf9…`）：Codex `0×P0 + 1×P1 + 1×P2`、Claude `0×P0 + 1×P1 + 4×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r29 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點（以實測命令核對，不採信修訂歷史自述）：

1. **bridge session 身份交接（Codex P1）**：`rg -n 'resolve_session_compiler_identity|bridge_session_hmac|bridge_session_bound_identity_matrix' plan-20260819.md` 是否同時命中 ADR/表職責、M2-02 A1（八列）、M2-08 AC5、M2-13 handle 文字、M2-16C AC + VER；M2-16C 是否 `AC=33/8@EX-M2-07`、`VER=11/8@EX-M2-06`，判據塊 B0–B10 共 11 門且與 11 個 checkbox 1:1；`EX-M2-07`/`EX-M2-06` 理由欄、全域口徑、審計表逐處一致。
2. **waiver 理由同步（Claude P1）**：EX-M2-07 理由是否 33 且含 ActionClass；EX-M2-06 含 approval；EX-M2-05「內容性」；EX-M2-04「五型」。
3. `rg -n '`[^`]*\\\|[^`]*`' plan-20260819.md` 零命中；表格機械列數；審計表末行空行；`**Verification:**` 空行統一。
4. review log 的 Scope 欄輪次 = Round − 1（新增機械檢查註記）；`plan-status` r18–r29 完整；25 卡逐欄、基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r31 附加要求（第三十一次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r30 修訂後的複審。r30 結果（同一凍結版 plan sha256 `b78d3c89…`）：Codex `0×P0 + 1×P1 + 1×P2`、Claude `0×P0 + 3×P1 + 5×P2`；修訂決策見計畫「修訂歷史」2026-09-21 r30 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 重點（以實測命令核對，不採信修訂歷史自述）：

1. **bridge 認證簽發面**：`rg -n 'bridge_session_hmac|resolve_session_compiler_identity|register_compiler_identity' plan-20260819.md` 是否同時命中表職責（八欄）、ADR-M2-05、§0.3、M2-02 A1/VER、M2-08 AC5（五方法集合）、M2-13、M2-16C AC；AC 是否明寫「已認證 ingress 以 vault/config 派生 principal 呼叫、請求體自報 principal/identity 一律拒絕、`resolve_session_compiler_identity` 僅查驗不創建」。
2. **計數自洽**：M2-16C `AC=38/8@EX-M2-07`、`VER=11/8@EX-M2-06`；判據塊算式逐項相加 = 38 且 JSON 分組齊全（含 AC3d(6)）；B0–B10 共 11 門與 11 個 checkbox 同序；`EX-M2-07` 理由欄 38 且含 bridge session；全域口徑、Granularity、審計表一致。
3. `EX-M2-09` AC1-1 是否為 `MemoryRecordEnvelopeV1`；M2-08 AC5-⑦ 是否「五方法」。
4. review log Scope 是否為 R30-r30（無 `r36` 類筆誤）；`plan-status` r18–r30 完整；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r32 附加要求（第三十二次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r31 修訂後的複審。r31 結果（同一凍結版 plan sha256 `e1b87f8b…`）：**Codex `PASS`（0×P0/0×P1/2×P2）**、Claude `FAIL`（0×P0/2×P1/5×P2）；修訂決策見計畫「修訂歷史」2026-09-21 r31 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。** 首要目標是讓本輪的兩個評審人對同一凍結版都給出字面 `VERDICT: PASS`。重點（以實測命令核對，不採信修訂歷史自述）：

1. **identity retention（Claude r31 P1-1）**：`memory_compiler_identity` 的 TTL 30 天／每倉 ≤1,000 行／原子 prune／引用保護／fail-closed 是否落在 **M2-08 AC5 與 `job_sql.rs` 寫集**；表職責 `:329` 是否寫出邊界；AC5(17)、`EX-M2-09` 分子 38、全域口徑、Granularity、審計表逐處一致；`job_crash_recovery_retention_and_identity_matrix` 是否覆蓋 identity prune 分支。
2. **principal 派生 owner（Claude r31 P1-2）**：`authenticated_repo_principal(repo, worktree)` 是否為 **M2-01K** 具名交付（寫集/AC 9/VER 7/`prod-files=7`、審計 `9/8@EX-M2-01`/`7/8`/`3/7`）；`:997`/`:532`/`:2392` 是否直接引用該函數；M2-16C 與 M2-13 的 `Dependencies` 是否列入 M2-01K 且 DAG 可達；缺失時穩定拒絕與具名 VER 是否具備。
3. M2-16C 寫集括注是否與 AC3d 的三個 `MemoryRuntime` 方法一致；`EX-M2-09` ⑦ 是否「五方法」；8 行審計 `axis` 是否逐字等於卡內；`EX-M2-09` AC1-1 與事實基線 producer 措辭。
4. review log Scope 是否為 R30-r31、`plan-status` r18–r31 完整；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r33 附加要求（第三十三次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r32 修訂後的複審。r32 結果（同一凍結版 plan sha256 `977a76fc…`）：Codex `0×P0 + 2×P1 + 3×P2`、Claude `0×P0 + 2×P1 + 5×P2`（兩方一致：純機械同步項）。修訂決策見計畫「修訂歷史」2026-09-21 r32 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。目標是同一凍結版上兩位評審同給字面 `VERDICT: PASS`。** 重點（實測命令核對）：

1. **EX-M2-09 分子**：waiver 補償句、理由、判據塊（38 條 = AC1(2)+AC2(4)+AC3(4)+AC4(3)+AC5(17)+AC6(2)+AC7(3)+AC8(3)）、Granularity、審計表 M2-08 行**五處皆 38**；`rg '34/8@EX-M2-09'` 零命中。
2. **M2-01K / EX-M2-10**：卡內 AC checkbox 是否 **9**（第 9 條具名 `authenticated_repo_principal`，含禁止請求體自報與缺失穩定拒絕）、VER **7**、Granularity `AC=9/8@EX-M2-10`、`exception=EX-M2-10`、審計三欄（`9/8@EX-M2-10`/`7/8`/`3/7`/`EX-M2-10`）；`EX-M2-10` 行是否 8 列且理由/補償/分子自洽；M2-16C AC3d 是否直接引用該函數；M2-16C/M2-13 的 Granularity deps、審計 deps、DAG 邊（`M201K→M216C/M213`）是否一致。
3. **矩陣與格式**：`job_crash_recovery_retention_and_identity_matrix` 是否含 identity TTL/行數裁剪、引用保護拒絕、prune fail-closed 三分支；`AC5-n` 標籤與斷句；審計 M2-16A `axis` 逐字；表職責引用保護；§0.2 producer 措辭與 `:372` 一致。
4. review log Scope = Round − 1；`plan-status` r18–r32 完整；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r34 附加要求（第三十四次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r33 修訂後的複審。r33 結果（同一凍結版 plan sha256 `b7c09faf…`）：Codex `0×P0 + 1×P1 + 2×P2`、Claude `0×P0 + 4×P1 + 5×P2`（全部為落點類機械項）。修訂決策見計畫「修訂歷史」2026-09-21 r33 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。目標：同一凍結版上兩位評審同給字面 `VERDICT: PASS`。** 重點（實測命令核對）：

1. **DAG 邊落點**：`M201K --> M216C`／`M201K --> M213` 是否僅出現在「實施順序」mermaid（第 3 個），且第 1 個架構圖無此二邊、無懸空節點。
2. **M2-01K / EX-M2-10**：Granularity `AC=9/8@EX-M2-10`；卡內 `EX-M2-10` 判據塊（K1–K3）存在；`Files likely touched` 7＋`principal.rs`；審計 `9/8@EX-M2-10`/`7/8`/`3/7`/`EX-M2-10`。
3. **審計表 M2-13/M2-16C**：`scope` 欄是否為 `4/5`/`4/7`，`deps` 欄是否含 `M2-01K(+principal)`，且與卡內 Granularity 逐字一致（15 欄）。
4. **格式**：`EX-M2-09` AC5 標籤是否全部 `AC5-n`；M2-16C→M2-16D 標題前空行；§0.2 producer 措辭；review log Scope = Round − 1；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；`plan-status` r18–r33 完整；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r35 附加要求（第三十五次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r34 修訂後的複審。r34 結果（同一凍結版 plan sha256 `93415b73…`）：Codex `0×P0 + 1×P1 + 3×P2`、Claude `0×P0 + 1×P1 + 4×P2`（同一 P1：審計 `scope` 格）。修訂決策見計畫「修訂歷史」2026-09-21 r34 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。目標：同一凍結版上兩位評審同給字面 `VERDICT: PASS`。** 重點（實測命令核對）：

1. **審計 `scope` 格**：M2-13=`M`、M2-16C=`M`，且 landing/prod 欄仍 `4/5`/`4/7`、deps 欄含 `M2-01K(+principal)`；15 欄與卡內 `Granularity` 逐格一致。
2. **`EX-M2-09` 標籤**：`AC5-8`/`AC5-9`/`AC5-10` 三條分離且全塊 `AC5-1..AC5-17` 唯一；分子 38 不變。
3. **卡邊界空行**：`rg -U '\S\n### Task M2-'` 零命中（含 M2-16C→M2-16D、M2-08→M2-09）。
4. **`EX-M2-07` 分組**：是否按 checkbox 序（AC3 bridge session → AC10），總和仍 38；`**Files likely touched:**` 標籤後皆有空格的機械檢查（`rg '^\*\*[^*]+:\*\*\S'` 零命中）。
5. review log Scope = Round − 1；`plan-status` r18–r34 完整；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r36 附加要求（第三十六次複審；本輪評審期間凍結計劃檔與模板）

本輪是 r35 修訂後的複審。r35 結果（同一凍結版 plan sha256 `0cd9c198…`）：**Claude `PASS`（0×P0/0×P1/3×P2）**、Codex `FAIL`（0×P0/1×P1/1×P2：minor/capability 會話隔離未定義）。修訂決策見計畫「修訂歷史」2026-09-21 r35 行。**被審版本 SHA-256 見 log 檔頭（計畫與模板皆凍結）；本輪期間不再變更。目標：同一凍結版上兩位評審同給字面 `VERDICT: PASS`。** 重點（實測命令核對）：

1. **協議會話隔離**（Codex r35 P1）：M2-16C 是否明寫握手 `protocol.minor`/`capabilities`、session 級 method 集、dispatch 強制、1.0 peer 只得 20、1.2 peer 得 22、舊 peer 調用新 method 穩定拒絕；`agent_bridge_protocol_test::protocol_v1_0_peer_gets_only_20_methods` 與 `unnegotiated_memory_method_rejected` 是否為具名 VER；B11/B12 是否與 checkbox 同序；AC=41/8@EX-M2-07、VER=13/8@EX-M2-06、算式、waiver 理由/分子、全域口徑、Granularity、審計表逐處一致。
2. **`EX-M2-09` AC5 明細**是否為獨立 bullet（17 條，位於 AC4 與 AC6 之間），導語只保留總式；分子 38 不變。
3. **§0.2 :25** producer 證據是否改為可復算的具名 `rg` 命令。
4. review log Scope = Round − 1；`plan-status` r18–r35 完整；表格機械列數、審計表空行、`**Verification:**` 空行、`rg -U '\S\n### Task M2-'` 零命中、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。

輸出規則同前：P0/P1/P2、位置/問題/為何重要/最小修復；最後一行恰為 `VERDICT: PASS` 或 `VERDICT: FAIL`。

## R30-r37 附加要求（第三十七次複審；本輪評審期間凍結計劃檔與模板/狀態檔）

本輪是 r36 修訂後的複審。r36 結果（同一凍結版 plan sha256 `567dfc11…`）：**Claude `PASS`（0×P0/0×P1/5×P2）**、Codex `FAIL`（0×P0/1×P1/2×P2：`plan-status.md` 合併行、M2-08 空行）。修訂決策見計畫「修訂歷史」2026-09-21 r36 行。**被審版本 SHA-256 見 log 檔頭（計畫、模板與 `plan-status.md` 皆凍結）；本輪期間不再變更。目標：同一凍結版上兩位評審同給字面 `VERDICT: PASS`。** 重點（實測命令核對）：

1. **`plan-status.md` 逐卡粒度**：`M2-01K..M2-14C` 合併行是否已拆為 17 張卡各自一行（各自 `pending`）；總行數與 25 卡一致；M2-16D1 條件狀態單列保留。
2. **卡邊界空行**：`rg -U '\S\n### Task M2-'` 零命中（含 M2-08→M2-09、M2-16C→M2-16D）。
3. **M2-16C 協議落點**：AC9 是否寫「既有 `protocol.minor`/`BridgeCapability` 賦予協商語義、`PROTOCOL_MINOR 0→2`」；寫集是否含 `transport.rs` 且 `prod-files=8`、審計 `4/8`；`EX-M2-07` 分組序（AC11 位於 AC4 後）與 `EX-M2-06` B0–B12 是否與 13 個 checkbox 同序；`Files likely touched` 是否點名 `transport.rs`/`PROTOCOL_MINOR`。
4. review log Scope = Round − 1；`plan-status` r18–r36 完整；表格機械列數、審計表空行、`**Verification:**` 空行、可複製命令無 `\|`；25 卡逐欄與基線事實/版本面/CI job/migration tip 不變。
