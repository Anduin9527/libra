...
- [ ] disabled (key-flag/self-signature) 被拒绝。　**（残余缺口：fixture 集无 disabled 证书；其余四类（revoked key/subkey、expired subkey、issuer 冲突、Sign 能力）均有具名门且绿。）**
...
- [ ] 五类拒绝 + primary-only fixture 与 ADR-VG-09 追踪表一一对应　**（4/5 类有具名门：`revoked_key_is_rejected_at_selection_time`、`revoked_subkey_is_rejected`、`expired_subkey_is_rejected_at_evaluation_time`、`issuer_conflict_subkey_is_rejected`、`secret-primary-only.asc` 门；disabled 类缺 fixture。）**
