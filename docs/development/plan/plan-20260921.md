...
- [ ] 手工证据：真实 gpg primary-only 与 subkey 证书各一，存 `/tmp/issue-vg/vg11/`　**（残余缺口：该手工证据未产生；等价自动门为 `expired_subkey_is_rejected_at_evaluation_time`、`revoked_subkey_is_rejected`、`issuer_selects_trusted_subkey`。）**
