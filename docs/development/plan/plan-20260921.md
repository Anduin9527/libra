...
**门 33（权威全量，终局）：** `FMT_EXIT=0`/`CLIPPY_EXIT=0`/`NEXTEST_EXIT=0` — **`Summary [451.422s] 7972 tests run: 7972 passed (14 slow), 3 skipped`，`FAIL=0`、`FLAKY=0`**；口径与门 32 相同（两门差异仅 `merge_verify_signatures_*` 的落点由 `command::merge_test` 移入 `command::merge_test::gpg_sign`，以满足 VG-05 写集）。
