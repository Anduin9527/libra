...
> **模板版本:** `v2.6`。文件原名 `plan-20260919-gpg-import.md`，2026-09-21 改名为 `plan-20260921.md`。**评审已通过（2026-09-19，R29 双 PASS）；Phase 0 已收口（2026-09-20）——实施已展开（2026-09-22 起，无 Codex/Claude review，操作者指示）**：VG-00 `done/complete`；家族子卡 VG-01/VG-10/VG-02/VG-11/VG-03/VG-13/VG-12/VG-04/VG-05 与管理面 VG-06/VG-07/VG-08 的 **生产代码 + 单元/集成测试已合入**（commit `8aed55e`、`77d729f`、`04ee0f8`，全部 `Lifecycle=in-progress`、`Acceptance=locally-accepted`，待 VG-09 转 `done`）；VG-14 仅登 producer（版本化键名 + 错误传播），staged (a)–(e) 迁移与故障注入仍 `pending`；VG-09 发布未执行（需操作者授权 `push` + `gh release` 与 `../libra-backend` 网站同步）。文档已同步：`config/tag/merge/commit` EN+zh、`docs/development/commands/{config,tag,merge,commit}`、`COMPATIBILITY.md` config 行；`push.md` 与网站五页待同步。所有未执行任务卡、ER-13 全量门及完成判据的全量收口门一律运行 `source .env.test && source .env.live-test && cargo nextest run --all --no-fail-fast --retries 2`。`cargo test --all` 只供诊断，不能当通过证据。
>
