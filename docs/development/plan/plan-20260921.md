...
**对账数字最终：声明 63 项 → 已实现 **51 项**（含本轮新增的 `merge_gpg_sign_uses_imported_key`），仍缺 **12 项**；另额外交付 2 张计划未报名但同轴的门（`revoked_key_is_rejected_at_selection_time`、`subkey_choice_is_deterministic_by_key_id` 等）。**
