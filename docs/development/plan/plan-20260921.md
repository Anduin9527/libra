...
**对账数字更新：声明 63 项中，已实现 19 项（10 原有 + 2 导入端到端 + 9 本轮），**仍缺 43 项**（仍适用前方 (A)/(B) 两条出路）。
...
**对账数字终：声明 63 项 → 已实现 **55 项（87%）**，仍缺 **8 项**：**

**对账数字二次更新：声明 63 项中，已实现 **31 项**（10 原有 + 2 导入端到端 + 9 第一轮 + 12 第二轮，部分声明名合并），**仍缺 31 项**。**
...
**最终对账（自动化重算）：声明 63 项 → 已实现 **50 项（79%）**，仍缺 **13 项**：**
| 仍缺的 13 项 | 分类与为何未实现 |
|---|---|
