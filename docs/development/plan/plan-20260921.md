...
- [x] 收口门三连全绿（终局修复后重跑）：`cargo +nightly fmt --all --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`source .env.test && source .env.live-test && cargo nextest run --all --no-fail-fast --retries 2`。**2026-09-22 状态：** fmt/clippy ✅；nextest 全量曾跑出 **7858 passed / 1 failed**（`compat_serial_registry`，根因为本计划新增两张 `#[serial(cwd)]` 测试缺注册表行）；已修复（`8e52577`）并单独验证该 target **25/25 绿**；**终局全量绿门已完成：`GATE_EXIT=0`，`Summary [437.384s] 7861 tests run: 7861 passed (14 slow), 3 skipped`（含本轮 4 张 GPG 集成测试均 PASS）**。　**（历史覆盖：门 21 全量 7931/7931、门 22 全量 7944/7944、**门 23 全量 7954/7954**、**门 24/25/26 定向绿**、**门 27 全量（lib+command_test）6801/6801** + `fmt --check` + clippy `-D warnings` 全绿；**门 29 定向 4/4（G9–G12 注入）**、**门 30（`--all` 权威全量 = `FMT_EXIT=0`/`CLIPPY_EXIT=0`/`NEXTEST_EXIT=0`，`7968 tests run: 7968 passed (15 slow), 3 skipped`）见「门 24–30」记录**；`7964 tests run: 7964 passed (14 slow), 3 skipped`）见下方门 24–27 记录末段**；**门 23（2026-09-23，本轮最终树）** 见上文「门 23」记录。）**
