...
- [ ] 历史顺序确定（指纹字典序）。　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门（G8/G15 未实现）；解析失败不 panic 的等价覆盖见 `verification_ignores_source_with_internal_fixture` 的同族 fixture 解析路径。）**
...
- [ ] `source .env.test && cargo test --lib internal::vault -- generated_path_legacy_fallback_when_key_name_absent`（new：`vault.gpg.generated_key_name` 缺失时回退遗留 `libra-signing`；本卡只实现 read　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门；等价覆盖见「本轮补门成果」/「第二轮补门成果」与 `config_remove_gpg_key_and_force_message_is_pinned`。）**er 的 legacy 回退，producer/新鲜生成断言归 VG-14）
...
- [ ] 历史顺序/去重幂等；解析失败继续不 panic。　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门（G8/G15 未实现）；解析失败不 panic 的等价覆盖见 `verification_ignores_source_with_internal_fixture` 的同族 fixture 解析路径。）**
...
- (i) **`source` 无关性未测**：无 `verification_ignores_source_with_internal_fixture` 等价门；代码上验证只经 `verify_pubkey_allowlist()`（活动公钥 + 历史），不读 `vault.gpg.source`，属**代码可读但无门**。
- (ii) **issuer 缺失时的有界尝试未实现（实现与验收标准不符）**：`src/internal/vault.rs:1713-1720` 在 `issuers.is_empty()` 时对**允许列表全部公钥 × 全部 subkey 无上限尝试**（无计划要求的 16 上限与 fail-closed），存在“候选膨胀→验证成本随历史线性增长”的偏差；`verify_pubkey_allowlist()` 的规模由 `vault.gpg.history` 决定，攻击面限于本仓配置写入者，风险低但**契约未满足**。
...
- (iv) merge 侧的 imported-key 正/负用例（`..._accepts_imported_key`/`..._rejects_foreign_key`）未实现；现有 merge 门只覆盖「已签名 vs 未签名」。
- (v) 手工证据 `/tmp/issue-vg/vg05/` 未产生（本轮为自动化证据优先）。
...
**Verification:**
- [ ] `source .env.test && cargo test --test command_test config_test`（new：G1–G8 对应具名用例）　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门；等价覆盖见「本轮补门成果」/「第二轮补门成果」与 `config_remove_gpg_key_and_force_message_is_pinned`。）**
- [ ] `source .env.test && cargo test --test command_test config_test -- config_remove_gpg_key_inject_failure_{history,delete_imported,restore_pubkey,source}`（new，四步移除故障注入）　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门；等价覆盖见「本轮补门成果」/「第二轮补门成果」与 `config_remove_gpg_key_and_force_message_is_pinned`。）**
- [ ] `source .env.test && cargo test --test command_test commit_test -- imported`（回归回落路径）　**（残余缺口，如实登记：门 23 全量 7954 项中无该具名门；等价覆盖见「本轮补门成果」/「第二轮补门成果」与 `config_remove_gpg_key_and_force_message_is_pinned`。）**
