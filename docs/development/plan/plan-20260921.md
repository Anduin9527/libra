...
- [ ] `source=generated` 且无 `generated_pubkey` 时先写入快照。　**（无独立具名门；该步由生成路径先行调用 `snapshot_active_key_to_history_with_conn`，历史不变量门 `replace_never_deletes_history_rows` 覆盖同族语义。）**
...
**门 31（权威全量，终局）：** `FMT_EXIT=0`/`CLIPPY_EXIT=0`/`NEXTEST_EXIT=0` — **`Summary [453.337s] 7969 tests run: 7969 passed (14 slow), 3 skipped`，`FAIL=0`、`FLAKY=0`**（7969 = 门 30 的 7968 + 1）。
