...
**对账数字终：声明 63 项 → 已实现 **52 项（83%）**，仍缺 **11 项**：**
...
| `revoked_subkey_is_rejected`、`issuer_conflict_subkey_is_rejected`（2；`expired_subkey_is_rejected_at_evaluation_time` 已实现并绿） | 需**已吊销/已过期 subkey**的签发与验证 fixture：宿主 `gpg 2.4.9` **无** `--quick-revoke-subkey`，`--quick-revoke-uid` 不能代替；已领取 **整钥** 吊销（fixture `secret-revoked-key.asc`）并已补门 `revoked_key_is_rejected_at_selection_time`（修复后绿），但**子钥级**吊销仍缺 fixture；过期 subkey 待生成（`--faked-system-time` 尝试失败）；`issuer_conflict_subkey` 需构造多 issuer 签名 |
| `tty_prompt_collects_passphrase`、`passphrase_and_plaintext_buffers_are_zeroized`（2；仍未实现） | 需 **pty 交互**或**堆 sentinel 捕获**（后者为本机安全考虑不引入） |
