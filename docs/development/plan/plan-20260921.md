...
| `merge_gpg_sign_uses_imported_key`（已实现已绿）、`push_certificate_payload_uses_imported_signing_key`（1） | merge 侧已补门（分叉 + 导入密钥 + `commit.gpgSign` → merge 提交带 `gpgsig`，PASS）；push certificate 载荷需配置**远端与证书**全场景（已有生成密钥版本可比） |
