...
**残余缺口（如实登记）：** 仅 VG-04 的 `generated_path_legacy_fallback_when_key_name_absent` 尚无端到端具名门；其行为由 `generated_key_name()` 的 reader-side 回退（`unwrap_or(PGP_KEY_NAME)`）承担，属 VG-14 已覆盖路径。
