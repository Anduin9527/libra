...
**★权威计数（最终口径，取代本文件其余各轮快照）：声明 63 项 → 已实现 **57 项（90%）**，仍缺 **6 项**：**
...
| `issuer_conflict_subkey_is_rejected`（1；`revoked_subkey_is_rejected` 与 `expired_subkey_is_rejected_at_evaluation_time` 已实现并绿） | 需**已吊销/已过期 subkey**的签发与验证 fixture：宿主 `gpg 2.4.9` **无** `--quick-revoke-subkey`，`--quick-revoke-uid` 不能代替；已领取 **整钥** 吊销（fixture `secret-revoked-key.asc`）并已补门 `revoked_key_is_rejected_at_selection_time`（修复后绿），但**子钥级**吊销仍缺 fixture；过期 subkey 待生成（`--faked-system-time` 尝试失败）；`issuer_conflict_subkey` 需构造多 issuer 签名 |
...
| `push_certificate_payload_uses_imported_signing_key`（1） | **本测试环境不可实现（已核实）**：`--signed` 要求远端在发现阶段广告 `push-cert[=<nonce>]`（`resolve_push_cert_nonce`，`src/command/push.rs:561-577`），而 `push-cert` 只出现在 `src/command/push.rs`，**本地传输客户端从不广告该能力**（`src/internal/protocol/local_client.rs` 仅广告 `symref` 等）→ 需要真实服务器/远端支持；`merge_gpg_sign_uses_imported_key` 已实现并绿 |
