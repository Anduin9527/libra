...
**门 35（权威全量，终局）：** `FMT_EXIT=0`/`CLIPPY_EXIT=0`/`NEXTEST_EXIT=0` — **`Summary [443.958s] 7978 tests run: 7978 passed (14 slow), 3 skipped`，`FAIL=0`、`FLAKY=0`**（7978 = 门 33 的 7972 + 6：VG-14 G1–G6 注入门；其间门 34 首跑抓到一次真实回归——resume 分支一度被放宽而误吞正常迁移路径、打破 `config_generate_gpg_key_after_import_same_nanosecond_collision_retries`，已撤回为窄条件并复绿）。
