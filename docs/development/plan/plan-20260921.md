...
**执行期澄清（2026-09-22，操作者指示「不调用 Codex/Claude review」下由执行者登记）：** VG-14 的 6 门逐步故障注入（`inject_failure_after_{a..e}`）与 ADR-VG-07 §2「**不新增 debug env hook**」冲突：`config_generate_gpg_key_after_import_inject_failure_after_*` 属 CLI 集成门，需在 `target/debug/libra`（非 `cfg(test)` 构建）中可用的 per-step 注入点，而能按步定位 config 写入的唯一机制是运行期 env failpoint（ADR-VG-07 §2 禁止；R25 亦以 P1 移除了 VG-02 的 env hook）。外部注入（如把配置库改只读）会同时命中 (a) 历史行写入，无法按步定位。因此这 6 门在现行计划文本下**不可实施**，需二选一：① 计划修订显式授权 `LIBRA_TEST` 门控的 per-step failpoint；② 把这 6 门改述为 lib 级 `#[cfg(test)]` 注入测试（不经 CLI）。**澄清落地前 VG-14 保持 `Lifecycle=in-progress` / `Acceptance=locally-accepted`，`inject_failure_after_{a..e}` 与 collision/retry-exhaustion 两类门记为 `pending`（blocked-by-plan-conflict）。** 已交付的 VG-14 实现面：纯 `vault_generate_pgp_key` helper、版本化键名 + 纳秒碰撞 probe（≤8 次）、staged (a)–(e) 逐步写入 + 逐错误传播、(e) 失败后 resume 幂等（不铸第二把键）、迁移集成测试与 resume 集成测试。
...
| 再生成迁移（VG-14） | internal::vault 3 门（G9/G10/G12）——**待 auth failpoint 澄清（blocked-by-plan-conflict，见 VG-14 卡执行期澄清）** | config_test：已绿 **G7 迁移（`config_generate_gpg_key_migrates_imported_key_into_history`）** 与 **resume 幂等（`..._resumes_idempotently`）**；**G1–G6、G8（`inject_failure_after_{a..e}` 与纳秒碰撞重试）blocked-by-plan-conflict**；G11（`fresh_generation_records_versioned_key_name`）未单独建门但被迁移/生成回归覆盖 | imported→generate 往返 |
...
- [ ] 所有非延后任务 `Lifecycle=done` 且 `Acceptance=complete`（家族子卡以 VG-09 完成为前提）。**2026-09-22 状态：** VG-00 `done/complete`；其余 14 卡 `in-progress`/`locally-accepted`（VG-09 未发布）；VG-14 的 6 门逐步故障注入 `blocked-by-plan-conflict`（见 VG-14 执行期澄清）。
