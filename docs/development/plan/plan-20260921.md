...
**★权威计数（最终口径，取代本文件其余各轮快照）：声明 63 项 → 已实现 **55 项（87%）**，仍缺 **8 项**：**
...
**最终对账（第3轮，已被第 4/5 轮取代，仅存历史）：** 声明 63 项 → 已实现 50 项（79%），仍缺 13 项：**
| 仍缺的 13 项 | 分类与为何未实现 |
|---|---|
| `generate_pgp_key_propagates_config_write_error`、`generate_pgp_key_retry_exhaustion_fails_closed`、`config_generate_gpg_key_after_import_same_nanosecond_collision_retries`、`import_partial_failure_rolls_back`、`config_import_gpg_key_failure_leaves_no_metadata`（5） | **C 类：需逐步故障/碰撞注入**，被 ADR-VG-07 §2「不新增 debug env hook」阻塞（另见 VG-14 卡执行期澄清：lib 级注入不可安全沙箱化） |
