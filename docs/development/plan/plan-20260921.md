...
1. **VG-09 家族发佈 + REL-VG-01 证据**：授权后按计划执行 `libra push origin main`（当前 `ahead 69, behind 3`，需先处置 behind 3 的远端分叉）→ 预建 annotated tag 并推送 → `gh release create --verify-tag` → `gh run view` 五字段 + 8 个 job 断言 → CDN gate + 安装冒烟。完成后把 VG-01..14 的 `Lifecycle` 转 `done`。
