...
**★权威计数（最终口径，取代本文件其余各轮快照）：声明 63 项 → 已实现 **59 项（94%）**，仍缺 **4 项**：**
...
**★仍缺的 6 项（与上方权威计数一致）及其受阻证据：**
| 仍缺的 4 项 | 分类与为何未实现 |
|---|---|
| `import_partial_failure_rolls_back`、`generate_pgp_key_retry_exhaustion_fails_closed`、`config_generate_gpg_key_after_import_same_nanosecond_collision_retries`（3） | **C 类：需逐步/碰撞注入**，被 ADR-VG-07 §2「不新增 debug env hook」阻塞（另见 VG-14 卡执行期澄清：lib 级注入不可安全沙箱化）。**注：** `config_import_gpg_key_failure_leaves_no_metadata` 与 `generate_pgp_key_propagates_config_write_error` 已实现并绿——前者用错误口令触发失败并断言零元数据，后者用**只读 DB** 诱导写失败（无需 env hook）并断言错误被传播 |
