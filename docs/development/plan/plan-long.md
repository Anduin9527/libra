...
| ID | 任务 | 优先级 | 状态 | 一句话缺口 |
|---|---|---:|---|---|
| **SB-02** | 统一 AI Tool / sandbox 信任边界 | P1 | 实施中 | 确定性权限引擎已落地（`src/internal/ai/permission/` Allow/Deny/Ask + wildcard + 持久 approvals + `sandbox`/`run_admission` fail-closed）；剩余跨源凭据作用域、URL 脱敏、写重定向 fail-closed 与团队发布门禁 | |
...
| SB-02 | 统一 AI Tool / sandbox 信任边界 | P1 | 实施中 | AgentRuntime、automation、Memory 机器面 |
...
| 日期计划 | 主要归属 | 当前状态 | 说明 |
|---|---|---|---|
| [`plan-20260925.md`](plan-20260925.md) | B（Session Capture 决策中层） | 已排期 | 把 AgentTraces ingest 的 `agent_session.state` 与 checkpoint 类别收成 `session_capture::decide`。前置对象是 OpenCode、Codex、Claude Code、Pi。写集含 `runtime.rs` 的卡见 DEP-SCAP-03。SCAP-02、SCAP-01 尚未执行 |
...
| [`plan-20260920.md`](plan-20260920.md) | 横切（拆除 `libra code` / Publish / Worker） | 已完成 | 公开 Code/Publish 表面已随 0.23.0 删除；内部 SCC、leftover、Code UI 测试面与 `worker/` 已删；RC-32 文档收口已随 closeout `1e74d0c` 完成 |
