...
For `bridge --stdio`, no method has a transport total-duration limit. The
inactivity timeout is 60 seconds: useful request-local progress renews it,
including nonempty thinking and text deltas during Episode compilation.
Operations without intermediate progress must respond within that idle window.
No initial output or a stalled operation still times out. The legacy handshake
field `request_deadline_secs` now describes this inactivity window. Streaming
and thinking remain enabled; workspace lease durations are unchanged.
