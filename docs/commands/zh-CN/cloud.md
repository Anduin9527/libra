...
| `LBR-IO-002` | cloud 操作前无法重放耐久的本地 object-index repair marker；若原因为锁超时，错误信息会指出锁持有者（pid 与用途）或说明无法判定持有者——等待该进程结束后重试，且不要删除 `.libra/object-index-repair-locks` 下的锁文件 |
