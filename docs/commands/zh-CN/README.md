...
| `libra am` | | 应用 plain-text format-patch 邮件，支持 continue/skip/abort 恢复 | [am.md](am.md) |
| `libra format-patch` | | 生成可由 Git 消费的 plain 或 MIME mbox patch series | [format-patch.md](format-patch.md) |
...
| `libra fast-export` | | 把多 ref/range 历史、tag 与 notes 导出为 Git fast-import 流 | [fast-export.md](fast-export.md) |
| `libra fast-import` | | 原子发布 refs 地导入 quoted/inline/tag/notes fast-import 流 | [fast-import.md](fast-import.md) |
...
| `libra bundle` | | 创建、校验、检查与 unbundle Git v2 bundle | [bundle.md](bundle.md) |
...
| `libra mailinfo` | | 从一封 email patch 提取 metadata、message body 和 patch 文本 | [mailinfo.md](mailinfo.md) |
...
| Git 命令 | Libra 状态 | 安全工作流 | 文档 |
|----------|------------|------------|------|
| `git send-email` | 没有 `libra send-email`；Libra 不执行 SMTP 投递 | 用 `libra format-patch` 生成，再用 stock Git 或其他 mailer 校验/发送 | [send-email.md](send-email.md) |
