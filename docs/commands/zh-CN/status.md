...
| `metadata_unavailable` | `metadata` | 仓库对象缺失、损坏、不可用，或对象读取在 ObjectReadBudget 墙钟期限内被取消（WIO-03：回收 out-of-process 对象读取 helper）；依赖它的 inexact 候选被跳过 |
| `metadata_budget_exceeded` | `metadata` | 内容读取预算或单对象大小上限触顶（2 MiB / 64 MiB / 64 objects / 5s 批次）；剩余候选被跳过 |
...
- **`--json`** 正常成功并报告部分结果：每个受阻路径都出现在 `data.io_blocked[]` 中，形态为 `{path: {display, raw_base64}, staged, reason, rename}`——`display` 是转义后的仓库相对形式（与非 `-z` porcelain 同一 quoting）；名字不是合法 UTF-8 时 `raw_base64` 承载精确 OS 字节（否则为 `null`）——Unix 为原始 `OsStr` 字节，Windows 为按小端序列化的 UTF-16 码元，因此未配对代理项同样可逆还原；`staged` 为已知的 staged 分量（`"M"`/`"A"`/`"D"`/`"R"` 或 `null`）；`reason` 为 `"permission_denied"`、`"io_error"` 或 `"io_timeout"`（单次文件系统操作超过 probe 截止时间——Libra 回收 out-of-process I/O worker 并保留最后 checkpoint 前的 partial，而不是挂起 `status` 调用方；`.gitignore` / `.libraignore` 查找不走 helper，仍用进程内线程池 deadline）。目录遍历以仓库根目录 fd/handle 为边界：Linux 使用 `openat2(RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS)`；其它 Unix 用 `openat(O_NOFOLLOW | O_DIRECTORY)` 逐级下降；Windows 以 `FILE_FLAG_OPEN_REPARSE_POINT` 打开并在 reparse point 上 fail-closed。check→open 之间把目录替换为外部 symlink 的竞态记入 `io_blocked[]`（`io_error`），绝不跟随。已知受影响的 staged rename 对时 `rename` 为 `{from, to, score}`。条目按 raw 路径字节升序排序并去重；每条同时产生一条 `worktree_*` 警告。
