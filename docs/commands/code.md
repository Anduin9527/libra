...
| Provider | | `--provider` | *(none — resolved at startup)* | AI provider backend (see Provider Backends below). Without it, the provider is resolved from the `--agent` binding, then the persisted `code.defaultProvider` config key, then credential detection (a configured key skips detection); zero configured keys exit 128, several exit 129. |
