...
- `src/command/` contains every `libra <subcommand>` (Git-compatible commands plus Libra-specific commands such as `automation`, `sandbox`, `review`, `investigate`, `cloud`, `upgrade`, `op`, `hydrate`, `sparse-view`, `layer`, `deps`, `alternates`, `metadata`, `media` (only with `--features fastcdc`), and `agent/*`). `code`, `code-control`, `graph`, `usage`, `publish` and `db` are no longer commands (removed in the 0.23.0 breaking release; external-agent capture is `libra agent`, repository backup is `libra cloud`).
- `src/internal/` holds core logic: AI stack in `internal/ai/` (`agent_bridge/`, `agent_run/`, `observed_agents/`, `completion/`, `session/`, `hooks/`, `sandbox/`, `permission/`, `review/`, `investigate/`, `sources/`, `automation/`, `history.rs`, `traces.rs`, …; the Code-era executor directories were removed by plan-20260920 RC-23/RC-22); Sea-ORM models in `internal/model/`; network clients in `internal/protocol/` (`git_client`, `https_client`, `ssh_client`, `lfs_client`, `local_client`); other core modules include `internal/db/` + `internal/db.rs` (migrations), `internal/sequencer/`, `internal/sparse/`, `internal/layer/`, `internal/deps/`, `internal/alternates/`, `internal/obliteration/`, `internal/upgrade/` (signed self-upgrade), `internal/worktree_io/`, `internal/operation.rs` (operation log v1), and `internal/workspace.rs` (workspace leases).
...
- Mark tests with keyed lanes (`#[serial(cwd)]`, `#[serial(env)]`, `#[serial(hash_kind)]`, or a named external key) via `serial_test` if they mutate shared state; every annotation that `tests/SERIAL_CLASSIFY.sh` judges `global`/`lane:*` needs a matching row in `tests/SERIAL_REGISTRY.tsv` (`none`-judged annotations carry no row and are deletion candidates; `compat_serial_registry` re-runs the classifier and fails on missing, dangling or mismatched rows), and `.config/nextest.toml` must be regenerated with `sh tests/NEXTEST_GROUPS.sh` after touching the registry. Keep async tests on Tokio (`#[tokio::test]`, or `flavor = "multi_thread"` when needed). (The Code UI harness, `code_ui_*` and `harness_self_test` targets were removed in plan-20260920 RC-30.)
