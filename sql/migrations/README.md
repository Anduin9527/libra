...
| `2026072304`  | `sparse_view_worktree_scope` | `2026072304_sparse_view_worktree_scope{,_down}.sql` (§C.4.1.1: `sparse_view` re-keyed to (worktree_id, ordinal) and the `sparse.enabled` config_kv key re-projected into the per-worktree `sparse_view_meta` table; legacy state adopts to main only when no linked HEAD row exists, else the up migration fails closed (CHECK guard) pending explicit `sparse-view clear`; down fails closed on linked rows and re-projects the main toggle back into config_kv) |
| `2026072401`  | `worktree_registry_v2` | `2026072401_worktree_registry_v2{,_down}.sql` (§C.7: capability marker table `worktree_registry_capability` — its presence makes pre-v2 binaries refuse the repository at connect time (future-schema fail-closed) before they can parse or recreate `worktrees.json`; the registry file itself moves to `{schema_version: 2, entries: [...]}` with per-entry persisted stable `worktree_id`, and the renamed top-level key makes a v1 parser fail closed as a second belt; down drops only the marker — W3-s1b lifecycle tables will add CHECK-guarded down refusal while v2-only state exists) |
