...
/// Return whether a local reference-table branch belongs in Operation v2
/// snapshots. Unknown and ordinary refs remain included; only an exact
/// Libra-owned ref whose policy opts out is excluded.
pub(crate) fn operation_snapshot_includes_branch(name: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn operation_snapshot_policy_excludes_only_the_exact_memory_branch() {
        ...
        for included in [
            ...
            assert!(operation_snapshot_includes_branch(included), "{included}");
        }
    }
