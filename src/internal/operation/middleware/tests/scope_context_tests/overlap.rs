...
async fn operation_scope_overlapping_mutations_keep_live_and_final_bindings_isolated() {
    // Run the request under a task-local request scope so this test cannot share
    // the process-wide REQUEST_SCOPE fallback with concurrently running tests; the
    // isolation under test is request-slot isolation, not fallback-stack ordering.
    crate::internal::worktree_scope::with_request_scope(None, async {
