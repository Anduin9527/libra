//! Local automation control file lifecycle (plan-20260920 RC-05).
//!
//! Neutral lock / token / scope / pid-liveness helpers shared by
//! `libra service` and the remaining Code control surface. `libra code
//! --control write` uses three files under the ACTING worktree's
...
use crate::{
    ...
    utils::util::{self, try_get_storage_path},
};

fn resolve_storage_root(working_dir: &Path) -> Option<PathBuf> {
